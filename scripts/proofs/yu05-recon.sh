...
if [ "$RI_CODE" = "404" ]; then
  ...
  echo "   ⚠ AND THAT IMPLIES A SNAPSHOT-FORMAT CHANGE. The members do not retain a trade history:"
  echo "   TradeNatsPublisher drains a queue and publishes, keeping only a count, and the gateway's"
  echo "   egress acks carry a kind byte, not trade detail. Retaining history in the clustered"
  echo "   service means new replicated state, which MUST be written into the snapshot (a new T_*"
  echo "   record and a SNAPSHOT_FORMAT bump) or a rebuilt member replays to a different state and"
  echo "   the cluster diverges. That is a deterministic-core change, not a plumbing job — it cannot"
  echo "   be rolled gradually, and it needs its own proof. Deliberately not done in passing."
