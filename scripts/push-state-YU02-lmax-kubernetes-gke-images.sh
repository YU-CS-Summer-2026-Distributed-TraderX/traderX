...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "usage: bash scripts/push-state-YU02-lmax-kubernetes-gke-images.sh <artifact-registry-prefix> [generated-root]"
  echo "example: bash scripts/push-state-YU02-lmax-kubernetes-gke-images.sh us-east1-docker.pkg.dev/traderx-501015/traderx"
  exit 1
fi
