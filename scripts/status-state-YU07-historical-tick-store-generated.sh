...
STATE_DIR="${GENERATED_ROOT}/code/target-generated/YU07-historical-tick-store"
...
echo "[status] state=YU07-historical-tick-store"
if [[ -d "${STATE_DIR}" ]]; then
  echo "[status] scaffold=generated"
  echo "[status] runtime=inherits-YU06-eod-price-production"
