...
echo "[info] ${STATE_ID} reuses the YU05-post-trade-compliance (=> YU04 => YU03 => YU02 => state-014)"
echo "[info] runtime harness; only trade-processor (EOD price producer) + position-service (EOD P&L"
echo "[info] consumer) images carry the overlay, plus the database-init ConfigMap gains three EOD"
echo "[info] tables, an eod-session-close CronJob, and a Grafana chain dashboard."
