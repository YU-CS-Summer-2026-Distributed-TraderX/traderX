...
STATE_DIR="${GENERATED_ROOT}/code/target-generated/YU05-post-trade-compliance"
...
echo "[status] state=YU05-post-trade-compliance"
if [[ -d "${STATE_DIR}" ]]; then
  ...
  echo "[hint] run: bash pipeline/generate-state.sh YU05-post-trade-compliance"
fi
