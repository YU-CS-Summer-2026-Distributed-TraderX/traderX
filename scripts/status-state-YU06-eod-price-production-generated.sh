...
STATE_DIR="${GENERATED_ROOT}/code/target-generated/YU06-eod-price-production"
...
echo "[status] state=YU06-eod-price-production"
if [[ -d "${STATE_DIR}" ]]; then
  echo "[status] scaffold=generated"
  echo "[status] runtime=inherits-YU05-post-trade-compliance"
  if [[ -x "${UPSTREAM_STATUS_SCRIPT}" ]]; then
    "${UPSTREAM_STATUS_SCRIPT}" "$@"
  fi
else
  echo "[status] scaffold=missing"
  echo "[hint] run: bash pipeline/generate-state.sh YU06-eod-price-production"
fi
