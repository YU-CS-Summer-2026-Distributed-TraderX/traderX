...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "usage: bash scripts/deploy-state-YU02-lmax-kubernetes-gke.sh <artifact-registry-prefix> [output-dir]"
  echo "example: bash scripts/deploy-state-YU02-lmax-kubernetes-gke.sh us-east1-docker.pkg.dev/traderx-501015/traderx"
  exit 1
fi
...
bash "${REPO_ROOT}/scripts/prepare-state-YU02-lmax-kubernetes-gke-manifests.sh" "${PREFIX}" "${OUTPUT_DIR}"
