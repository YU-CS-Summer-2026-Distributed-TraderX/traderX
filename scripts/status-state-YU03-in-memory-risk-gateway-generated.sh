...
STATE_DIR="${GENERATED_ROOT}/code/target-generated/YU03-in-memory-risk-gateway"
...
echo "[status] state=YU03-in-memory-risk-gateway"
if [[ -d "${STATE_DIR}" ]]; then
  echo "[status] scaffold=generated"
  echo "[status] runtime=inherits-YU02-lmax-kubernetes"
  if [[ -x "${UPSTREAM_STATUS_SCRIPT}" ]]; then
    "${UPSTREAM_STATUS_SCRIPT}" "$@"
  fi
else
  echo "[status] scaffold=missing"
  echo "[hint] run: bash pipeline/generate-state.sh YU03-in-memory-risk-gateway"
fi
