...
[[ -d "${OM}" ]] || {
  echo "[fail] generated order-matcher missing; run: bash pipeline/generate-state.sh YU14-listed-equity-options"
  exit 1
}
...
docker build ${YU14_PLATFORM:+--platform ${YU14_PLATFORM}} -f "${OM}/Dockerfile.cluster" \
