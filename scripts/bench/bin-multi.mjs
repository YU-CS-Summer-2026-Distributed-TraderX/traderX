...
class Conn {
  constructor(i, security) {
    ...
    this.base = (i + 1) * 1e12;   // distinct clOrdId space per connection (stays < 2^53)
    this.k = 0;
    this.offered = 0;
    this.completed = 0;
    this.up = false;
  ...
  onData(chunk) {
    ...
    while (this.acc.length >= 2) {
      ...
      const clOrdId = Number(this.acc.readBigUInt64LE(p + 8));
  ...
  fire(intendedMs) {
    ...
    this.offered++;
    this.sock.write(frameNew(this.side, this.account, this.security, QTY, PX_TICKS, clOrdId));
  }
...
async function main() {
  ...
  console.log(`bin-multi: ${SESSIONS} connections across ${GATEWAYS.length} gateways ` +
    ...
    `rate ${RATE.toFixed(2)}/s/conn (${(RATE * SESSIONS).toFixed(0)}/s offered), ${SECS}s, ` +
  ...
  await new Promise((resolve) => {
    const tick = () => {
      ...
      for (const c of up) {
        let fired = 0;
        while (c.k * period <= (now - startMs) && fired < 64) {
          c.fire(startMs + c.k * period); c.k++; fired++;
        }
      }
