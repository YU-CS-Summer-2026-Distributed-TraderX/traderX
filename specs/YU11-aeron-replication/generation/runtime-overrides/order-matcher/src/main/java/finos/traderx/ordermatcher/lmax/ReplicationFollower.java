...
public final class ReplicationFollower {
    ...
    public void start() {
        ...
        if (ackMode == ReplicationAckMode.DURABLE) {
            durableAckThread = new Thread(this::durableAckLoop, "blp-nats-durable-ack");
    ...
    public void stop() {
        ...
        if (durableAckThread != null) durableAckThread.interrupt();
        try {
    ...
    private void inject(Message msg) {
        ...
        lastJetsStreamSeq.set(streamSeq);
        ...
        if (disruptorSeq >= 0 && ackMode == ReplicationAckMode.ON_RING) {
            publishAck(disruptorSeq);
        }
    }

    private void durableAckLoop() {
        long lastAckedLocalSeq = -1L;
        while (running) {
            long durableLocalSeq = journaledSeq.getAsLong();
            if (durableLocalSeq <= lastAckedLocalSeq
                || !sequenceMap.read(durableLocalSeq, ackEntry)) {
                LockSupport.parkNanos(50_000L);
                continue;
            }
            publishAck(ackEntry.inputSeq);
            lastAckedLocalSeq = durableLocalSeq;
            sequenceMap.consumed(durableLocalSeq);
        }
    }

    private void publishAck(long disruptorSeq) {
        try {
            ackBuf.clear();
    ...
    private static long checksum64(byte[] data) {
        long hash = 0xcbf29ce484222325L;
        for (int i = 0; i < NatsJournalReplicator.RECORD_BYTES; i++) {
            hash ^= data[i] & 0xffL;
            hash *= 0x100000001b3L;
        }
        return hash;
