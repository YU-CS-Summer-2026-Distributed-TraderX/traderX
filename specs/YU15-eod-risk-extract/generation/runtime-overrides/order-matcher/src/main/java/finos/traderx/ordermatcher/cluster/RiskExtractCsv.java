...
final class RiskExtractCsv {
    ...
    static String render(final String cut, final Map<String, Mark> marks,
                         final Map<Integer, Counterparty> accounts, final Stamp stamp) {
        ...
        for (int i = 2; i < lines.length; i++) {
            ...
            seen++;
            body.add(row(lines[i], marks, accounts, stamp));
        }
    ...
    private static void preamble(final StringBuilder sb, final Stamp stamp, final int rows) {
        ...
        sb.append("# quantityConvention=signed net position in contracts (options) or shares (equity)\n");
