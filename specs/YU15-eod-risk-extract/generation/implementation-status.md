...
| Runtime | NATS, published-close database, producer Deployment, NetworkPolicy allowlist entry |
| Proof | `RiskExtractTest` (16 tests), `scripts/bench/yu15-risk-extract.sh` (7 steps) |
...
- **The kind kustomization does not include `gateway.yaml`** — inherited from the YU12 layout; the
  start script applies it separately.
