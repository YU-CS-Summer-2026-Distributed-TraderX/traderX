...
| FD-LOB12 | Replace an open order atomically in ONE sequenced command (ADR-058): validate shape, band and risk before mutating anything, keep the orderRef, keep queue priority only on a strict size-down at an unchanged price, and answer a rejected replace with the order left exactly as it was. Ingress: REST `POST /replace` and FIX `OrderCancelReplaceRequest` (G). |
