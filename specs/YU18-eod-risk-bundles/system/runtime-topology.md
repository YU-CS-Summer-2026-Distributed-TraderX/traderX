...
Bundle v2 adds a local, hash-pinned instrument-terms file. It uses the existing builder/coordinator/local mock and introduces no port or service. The standalone shared-example demo invokes an offline Java exporter test, then Python packaging/validation. HTTP draft-1 remains v1-only.
