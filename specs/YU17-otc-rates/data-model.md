...
Everything inherited from `YU16-cdm-instruments` is unchanged. This state adds one command type,
...
`SNAPSHOT_FORMAT` = 5; `MIN_READABLE_SNAPSHOT_FORMAT` = 3, unchanged.
...
## 6. The contracts artifact — `seq-<N>-contracts.csv`, schema 1
...
| Column | Source |
|---|---|
| `contractId` | `SW-<contractId>` from the cut |
| `accountId` | cut |
