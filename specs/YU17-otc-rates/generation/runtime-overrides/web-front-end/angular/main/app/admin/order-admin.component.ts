...
export class OrderAdminComponent implements OnInit, OnDestroy {
    ...
    columnDefs: ColDef<OrderRow>[] = [
        ...
        // YU17: the FORCE FILL column is deliberately gone. That route exists on the single-BLP
        // tier but not on the cluster tier, where the matching engine's book decides fills and an
        // operator can only cancel. A button that always fails is worse than no button: it tells a
        // demo audience the system can do something it cannot.
    ];
    ...
    ngOnInit(): void {
        ...
        // subscribeOrders (not raw subscribe): it normalises the bridge's `id` field to `orderId`,
        // without which every live update was dropped by the guard in applyOrderUpdate. undefined
        // = all accounts, which is this page's scope.
        this.orderUnsubscribeFn = this.orderAdminService.subscribeOrders(undefined, (order: OrderRecord) => {
    ...
    onCellClicked(event: CellClickedEvent<OrderRow>): void {
        ...
        if (event.colDef.colId === 'cancel') {
            this.orderAdminService.cancelOrder(orderId).subscribe({
                next: (res) => {
                    this.actionMessage = res.status === 'CANCELED'
                        ? { type: 'success', text: `Canceled ${res.orderId}` }
                        : { type: 'danger', text: `${orderId} was not canceled (already filled or unknown to the book).` };
                },
                error: () => {
        ...
        // No force-fill branch: the column is gone on this tier (see columnDefs above).
    }
