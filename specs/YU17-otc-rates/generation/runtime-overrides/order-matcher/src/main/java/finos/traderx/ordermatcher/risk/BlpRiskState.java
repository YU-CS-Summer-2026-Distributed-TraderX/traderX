...
public final class BlpRiskState {
    ...
    /**
     * Admit an OTC swap booking (YU17, ADR-063). Same ordered pipeline, same stable precedence and
     * the same idempotency table as an order — a DIFFERENT set of checks, because four of the
     * order pipeline's checks are not merely inapplicable to a swap, they are WRONG for one.
     *
     * <p>What is deliberately absent, and why:
     * <ul>
     *   <li><b>{@code notional = quantity × price × multiplier}.</b> The formula the order path
     *       uses is the whole reason this method exists. A swap's fixed rate is not a price — it
     *       is a term of the contract — so {@code quantity × rate} is 10,000,000 × 0.042 = 420,000
     *       of nothing at all, understating a 10mm exposure by ~24x. The notional IS the notional;
     *       it is converted to ticks and used directly.</li>
     *   <li><b>Security enabled / restricted / priced.</b> A swap has no symbol-table entry and no
     *       last trade (there is no book). Running those checks against the convention index that
     *       rides the {@code securityId} slot would test an unrelated number and refuse every
     *       booking.</li>
     *   <li><b>Position and concentration limits.</b> Both are projections of {@code (account,
     *       security)} quantity, the grain a swap does not have. Netting a receive-fixed and a
     *       pay-fixed leg to zero exposure is exactly the error this state exists to demonstrate;
     *       it must not be reintroduced inside the gate.</li>
     * </ul>
     *
     * <p>What remains is everything that is genuinely about the ACCOUNT rather than the
     * instrument: duplicate suppression, kill switch, account existence and enablement,
     * entitlement, a sane notional, the per-booking notional cap, and credit. Accepted notional
     * accrues to {@code executedNotional} immediately — a booked swap is not a resting order, so
     * there is no reservation to release later (D6: no lifecycle in this state).
     *
     * @param notionalTicks the notional in Px ticks (whole units × 1e6), so it is measured against
     *                      the same {@code maxOrderNotionalTicks} and credit limit as every other
     *                      instrument
     * @param contractRef   remembered against the key on acceptance, so a retried booking resolves
     *                      back to the ORIGINAL contract through {@link #existingOrderRef} instead
     *                      of creating a second one. Must be &gt; 0 — the table reads 0 as "no ref".
     */
    public RiskReason decideSwapBooking(long clientOrderKey, long principalKey, int accountId,
                                        long notionalTicks, int contractRef) {
        duplicateReplay = false;
        if (clientOrderKey != 0L) {
            RiskReason previous = previousDecision(clientOrderKey);
            if (previous != null) return previous;
        ...
        if (notionalTicks <= 0) return remember(clientOrderKey, 0, RiskReason.INVALID);
        if (notionalTicks > maxOrderNotionalTicks) {
            return remember(clientOrderKey, 0, RiskReason.ORDER_NOTIONAL);
        }
        if (executedNotional[accountSlot] + reservedNotional[accountSlot]
                > creditLimitTicks - notionalTicks) {
            return remember(clientOrderKey, 0, RiskReason.CREDIT_LIMIT);
        }
        if (clientOrderKey != 0L && !canRemember(clientOrderKey)) return decision(RiskReason.CAPACITY);
        executedNotional[accountSlot] += notionalTicks;
        return remember(clientOrderKey, contractRef, RiskReason.ACCEPTED);
    }
