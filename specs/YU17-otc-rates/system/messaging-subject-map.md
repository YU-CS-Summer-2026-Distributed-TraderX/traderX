# Messaging Subject Map: YU17-otc-rates

Every inherited NATS subject is carried forward with its existing contract, including the YU16
entries below. YU17 adds no subject, removes none and renames none (NFR-OTC06). A swap booking
travels the Aeron cluster ingress, not NATS, and produces no trade, no order-lifecycle transition
and no position — so the `/trades`, `/orders` and per-account families carry nothing new.

The one observable difference is content, on `risk.extract.ready`: the announcement gains four
fields naming the second artifact.

The Aeron Cluster channels and the gateway FIX endpoint are inherited from YU12 unchanged, with one
added `commandType` and one added egress ack kind on the existing templates.
...
## YU17 change: `risk.extract.ready` announces two artifacts

The subject, producer, consumer, delivery and every inherited field are unchanged. The payload
gains `contractsSchema`, `contractsUri`, `contracts` (the row count) and `contractsSha256`. Both
artifacts share the existing `consensusSequence`, `sessionDate` and `cutSha256`: one cut, one
instant, two files (ADR-064).

## YU17 change: `risk.extract.cut` carries a second section

The subject and its single-message delivery are unchanged. The cut moves to schema 2 and appends a
`#contracts` section after the position rows; every line above the marker is byte-identical to what
schema 1 rendered for the same state. The section is present even when the portfolio holds no
swaps, so a consumer can never read an absent section as an empty one.

## YU17: the swap path uses no subject at all

`POST /swaps` → Aeron cluster ingress (`TYPE_SWAP_BOOK`, template 1) → egress ack
(`KIND_SWAP_BOOKED`, kind 102). No NATS subject is involved in booking a swap, because a swap
produces nothing the read model carries: no trade to bridge, no order lifecycle to publish, no
position to project.
