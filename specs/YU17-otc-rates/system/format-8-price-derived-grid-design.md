...
**Status: MINTED 2026-08-25 (chip 4) — see §7b for the gate results, including V4's on-rig detonator half, which chip 3 could not run.** The mechanism is deployed on `traderx/cluster-node:yu17-format8` on `kind-traderx-yu12-cluster`, on a fresh epoch, and the five proofs that cover it are green: `yu17-fnma-collar`, `yu17-option-collar`, `yu17-fine-grid`, `yu17-book-retick` and `yu17-retick-determinism`. The detonator run also found two assertions in `yu17-book-retick` that could not discriminate — one that could never pass and one that could never fail — both repaired before the mint; without that this design would have been signed off on a proof whose grid readings were broken. See `format-8-mint-scope.md` §9 for the mint's full result. Originally written as design only: Written 2026-08-25 against the YU17 tip, after yaakov's
...
| **V4** — the detonator fails exactly `yu17-book-retick` and no other proof | **PASS off-rig, precisely.** A build with `rederiveIfEmpty` deleted from `onNewOrder` fails **exactly five arms, all in `PriceDerivedGridTest`** — jobs 1, 2 and 4, the counter's semantics, and jobThree's tick assertion — out of 467 tests. Nothing else moves: not `GridRestoreFormatTest`, not `BookGridDerivationTest`, not `LimitBookRetickTest`, not the snapshot round trips. That is the claim: no pre-existing proof covered the accident window. **On-rig half DISCHARGED 2026-08-25 (chip 4)**: `traderx/cluster-node:yu17-format8-detonator` — today's tree minus the single line `rederiveIfEmpty(book, e.securityId);` in `onNewOrder`, built through `build-cluster-image.sh` with `OM_DIR` pointed at a patched copy so the shared generated tree is never touched, and confirmed a different binary (`MatchingEngine.class` sha differs). Deployed on its own fresh epoch, all four cluster-node components repinned; `EXPECT=after` goes red at **step 5**, the re-derivation discriminator: `SELL @22.00 -> kind=1`, i.e. the 20× probe ACCEPTED where the minted build refuses it PRICE_COLLAR. `traderx_book_reticks{member="0"} 0` (exported, never moved) beside `traderx_band_reanchors{member="0"} 1` (the old mechanism answering) — so the red is attributable to the missing mechanism, not to a missing counter or a seeding failure. **The detonator also found two broken assertions in the proof it detonates** — one that could never pass, one that could never fail; see [`issues/resolved/the-book-retick-tickpx-assertions-could-neither-pass-nor-fail.md`](../../../issues/resolved/the-book-retick-tickpx-assertions-could-neither-pass-nor-fail.md). Both fixed before the mint, and the run above is the fixed proof. |
