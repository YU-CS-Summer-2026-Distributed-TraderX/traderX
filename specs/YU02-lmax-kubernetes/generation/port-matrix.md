# YU01 -> YU02-lmax-kubernetes Port Matrix

This matrix is the architecture-first forward-port map for carrying the `YU01-lmax-sequencer`
...
1. What is the durable `YU01` behavior we are preserving?
...
- Treat `specs/YU01-lmax-sequencer/generation/runtime-overrides/order-management-matcher/docker-compose.yml`
...
| Stateful storage semantics | `YU01` docs: journal dir, snapshot dir, projection checkpoint | Durable target: `specs/YU02-lmax-kubernetes/system/runtime-topology.md`, `requirements/nonfunctional-delta.md`, and Kubernetes manifest overrides | New Kubernetes design | Define PVCs or equivalent storage for journal/snapshot/checkpoint paths. This is not present as a durable manifest implementation in YU01 because the original runtime was compose-oriented. |
| Observability and readiness | `YU01` hot-path metrics + replay readiness semantics from `implementation-status.md` and `runtime-topology.md` | Durable target: `specs/YU02-lmax-kubernetes/generation/runtime-overrides/order-matcher/...` and manifest probes under Kubernetes/Tilt bases | Carry forward and expose | Keep LMAX metric families and ensure Kubernetes probes do not mark `order-matcher` healthy before replay/warm-up completes. |
| Price tick routing | `YU01` runtime topology: price ticks reach Gateway for sequencing | Durable target: `specs/YU02-lmax-kubernetes/contracts/contract-delta.md`, `system/runtime-topology.md`, service/runtime overrides | Carry forward | `014` backend network shape otherwise stays intact; the meaningful backend route change is that price input must be treated as sequenced ingress, not out-of-band matcher subscription. |
| Frontend and FDC3 invariants | `014` spec pack and Sail overrides under `specs/014-fdc3-intent-interoperability/generation/sail-overrides/*` | Preserve in parent-generated surfaces: `generated/code/target-generated/web-front-end/angular/...`, `fdc3-intent-interoperability/...` | Keep inherited | No YU01 donor material should overwrite these assets unless the LMAX port later introduces an explicit UI/interop requirement. |

## Compose-Only Assumptions To Replace

- `docker-compose.yml` service wiring from `YU01` is reference material, not the target implementation.
