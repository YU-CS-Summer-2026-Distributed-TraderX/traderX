...
public final class NatsJournalReplicator implements EventHandler<InputEvent> {
    ...
    // onEvent is single-writer. Payload slots stay owned by their matching future until the batch
    // drain observes the JetStream ACK, so the async jnats writer never races buffer reuse. This
    // removes the old Arrays.copyOf allocation while keeping the broker-publish ACK contract.
    private final byte[][] payloads = new byte[MAX_IN_FLIGHT][RECORD_BYTES];
    private final ByteBuffer[] payloadBuffers = new ByteBuffer[MAX_IN_FLIGHT];
    @SuppressWarnings("unchecked")
    private final CompletableFuture<PublishAck>[] inFlight = new CompletableFuture[MAX_IN_FLIGHT];
    ...
    NatsJournalReplicator(JetStream js, String subject) {
        ...
        for (int i = 0; i < MAX_IN_FLIGHT; i++) {
            payloadBuffers[i] = ByteBuffer.wrap(payloads[i]).order(ByteOrder.LITTLE_ENDIAN);
        }
    }

    @Override
    public void onEvent(InputEvent e, long sequence, boolean endOfBatch) {
        // The cap is normally drained immediately after the prior event. Keep this precondition so
        // a failed/exceptional drain can never let us overwrite a buffer still owned by jnats.
        if (inFlightCount == MAX_IN_FLIGHT) {
            drainInFlight(sequence - 1L);
        }
        int slot = inFlightCount;
        ByteBuffer b = payloadBuffers[slot];
        b.clear();
        b.putLong(e.seq);
        b.put(e.type);
        b.put(e.side);
        ...
        try {
            // jnats' acknowledged JetStream API still creates its own request/future objects per
            // publish; the public API has no caller-supplied reusable future. The live allocation
            // gate records that residual library budget. Our 64-byte payload and tracking arrays
            // are now reused with a fixed MAX_IN_FLIGHT bound.
            CompletableFuture<PublishAck> future = js.publishAsync(subject, payloads[slot]);
            if (future == null) {
                throw new IllegalStateException("JetStream publish returned no ACK future");
            }
            inFlight[inFlightCount++] = future;
        } catch (Exception ex) {
        ...
        if (endOfBatch || inFlightCount >= MAX_IN_FLIGHT) {
    ...
    private void drainInFlight(long seq) {
        for (int i = 0; i < inFlightCount; i++) {
            try {
                inFlight[i].get(PUBLISH_ACK_WAIT_MS, TimeUnit.MILLISECONDS);
            } catch (Exception ex) {
    ...
    /** Test/telemetry view of the actual follower watermark (never the solo publish fallback). */
    long followerAckedSeq() {
        return followerAckedSeq.get();
    }
