...
**Created**: 2026-06-29  
**Status**: Draft  
**Input**: Forward port of `YU01-lmax-sequencer` onto `014-fdc3-intent-interoperability`
...
- As a maintainer, I want `YU01` business-path semantics ported intentionally onto `014`, instead of a raw branch merge that drags along stale runtime assumptions.
...
- FR-LK03: The state SHALL forward-port the `YU01` LMAX hot path by replacing inherited matcher-path internals with the sequencer, single-threaded BLP, output ring, and gateway-role changes from `YU01`.
...
- FR-LK05: `trade-service` SHALL take the `YU01` Gateway/Receptionist role under the Kubernetes runtime.
...
- NFR-LK05: Validation must separate inherited `014` platform checks from new `YU01` LMAX-path checks.
...
- SC-LK02: The state pack documents the `YU01 -> YU02-lmax-kubernetes` forward-port strategy clearly enough to guide implementation.
