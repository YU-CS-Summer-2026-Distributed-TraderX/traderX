...
- `YU01-lmax-sequencer` is still the authoritative source for LMAX hot-path behavior.
...
1. Which `YU01` runtime overrides are still authoritative versus superseded in `YU02-lmax-kubernetes`?
2. Which Kubernetes manifests from the `014` line need stateful LMAX-specific changes?
3. Which runtime assumptions in `YU01` are compose-era only and must be redesigned for Kubernetes?
