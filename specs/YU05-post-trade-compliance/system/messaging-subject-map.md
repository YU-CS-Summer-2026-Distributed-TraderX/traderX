# Messaging Subject Map (State YU05)

YU05 adds no new NATS subjects — settlement, reconciliation, regulatory reporting, and TCA are all
REST. Its one new messaging touch point is a new *consumer*: `trade-processor`'s `PriceHistoryStore`
(TCA benchmark source) subscribes to the existing `pricing.*` feed.
...
- `pricing.<TICKER>`
  ...
  - consumer: frontend valuation streams, `trade-processor`'s `PriceHistoryStore` (TCA benchmark source)
  - delivery: `broadcast`
