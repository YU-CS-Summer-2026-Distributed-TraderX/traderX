...
public final class TradeOrder {
    ...
    public static TradeOrder fromEvent(OutputEvent e, SymbolTable symbols) {
        ...
        // YU05 (post-trade-compliance, ADR-022): the trade's OWN deterministic id, not the
        // order's — tradeIdFor(tradeSeq) already existed for exactly this, but was unwired here,
        // so every downstream projection (trade-processor) had no stable link back to the journal.
        payload.setId(OrderSnapshot.tradeIdFor(e.tradeSeq));
