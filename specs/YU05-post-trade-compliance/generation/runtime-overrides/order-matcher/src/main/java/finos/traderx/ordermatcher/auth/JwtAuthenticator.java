 ...
 * extraction — no external OIDC provider integration (would need live IdP infrastructure this
 * project doesn't have), but genuine cryptographic verification, not a shared-string stand-in
 * like the token headers every other endpoint in this state still uses. JDK crypto (`javax.crypto`)
 * + Jackson (already a dependency) only — no new dependency, same discipline as YU03's risk gateway.
 *
 * <p>Expected claims: {@code sub} (subject, required), {@code accounts} (array of entitled account
 * ids), {@code admin} (bool, grants cross-account access), {@code exp} (epoch seconds, optional —
 * absent means non-expiring, fine for this slice's shared-secret, no-revocation-list scope).
 */
public final class JwtAuthenticator {
    ...
    /** Accepts either the raw token or a full {@code Authorization: Bearer <token>} header value. */
