...
public final class ProjectorHandler implements EventHandler<OutputEvent> {
    ...
    // YU05 (post-trade-compliance, FR-PTC02): business days added to a booked trade's `created`
    // timestamp to get its initial settlement date. Kept in this handler (not LmaxEngine) since
    // it's only needed at the point a Trade row is built for the DB.
    private final int settlementTPlusDays;

    public ProjectorHandler(JdbcTemplate jdbcTemplate, SymbolTable symbols, int batchSize,
                            int queueCapacity, HotPathMetrics metrics, int settlementTPlusDays) {
    ...
    private static final String TRADE_COLS =
        "(id, accountid, security, side, state, quantity, price, created, updated, settlementdate)";
    ...
    private void insertTradesBatch(List<Trade> trades) {
        for (int start = 0; start < trades.size(); start += TRADE_INSERT_CHUNK) {
            ...
            StringBuilder sql = new StringBuilder(64 + (end - start) * 22)
                .append("INSERT IGNORE INTO trades ").append(TRADE_COLS).append(" VALUES ");
            Object[] args = new Object[(end - start) * 10];
            int a = 0;
            for (int i = start; i < end; i++) {
                ...
                args[a++] = t.getSettlementDate() == null ? null : new Timestamp(t.getSettlementDate().getTime());
            }
    ...
    // Package-private (not private) so ProjectorHandlerTest (YU05, FR-PTC02) can exercise the
    // settlement-lifecycle fields directly without needing the full DB-backed drain machinery.
    Trade toTrade(OutputEvent e) {
        ...
        // YU05 (post-trade-compliance, FR-PTC02/06): previously set straight to Settled here —
        // there was no settlement lifecycle at all. Now Processing until trade-processor's
        // SettlementService sweep (or a manual override) advances it once settlementDate passes.
        ...
        trade.setSettlementDate(plusBusinessDays(when, settlementTPlusDays));
        return trade;
    }

    /** Adds {@code days} business days (Mon-Fri) to {@code from}, skipping weekends. */
    private static Date plusBusinessDays(Date from, int days) {
        java.time.LocalDate date = from.toInstant().atZone(java.time.ZoneOffset.UTC).toLocalDate();
        int added = 0;
        while (added < days) {
