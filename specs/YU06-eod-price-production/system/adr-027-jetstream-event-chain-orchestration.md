...
prices → downstream jobs (position marks/P&L, then NAV, VaR, regulatory reports). The deck names
Airflow/Control-M as the industry tools. Something must sequence the stages and let ops see where a
delay is (deck 07: *a delay at any stage compresses all downstream processing time*).
...
```
session-close ─▶ [produce · quality · publish] ─▶ eod.prices.ready ─▶ position marks/P&L ─▶ eod.pnl.done ─▶ (VaR/NAV, later)
```
...
- **Plain k8s Jobs + an ordering controller** — viable, but needs a new controller and doesn't
  reuse the JetStream infra already present; the event chain is lazier and matches the deck's
  "each job subscribes to the prior job's completion" description directly. Rejected for v1.
...
existing JetStream setup; the chain extends by adding a subscriber (the teammate's VaR batch binds
to `eod.prices.ready`; a later NAV job binds to `eod.pnl.done`) without touching existing stages.
