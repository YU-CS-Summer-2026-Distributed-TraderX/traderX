...
bash generated/code/target-generated/scripts/start-state-lmax-kubernetes-generated.sh \
  --provider kind --without-sail
```

UI at **http://localhost:8080**. The script generates state, builds all images, loads them into
...
```bash
bash generated/code/target-generated/scripts/stop-state-lmax-kubernetes-generated.sh
...
`deploy-state-lmax-kubernetes-gke.sh` runs the prepare script then `kubectl apply -k`. The prepare
script (`scripts/prepare-state-lmax-kubernetes-gke-manifests.sh`) rewrites the base manifests for
GKE:
