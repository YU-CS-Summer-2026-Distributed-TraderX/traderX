# 009b -> lmax-kubernetes Port Matrix
...
- Port durable behavior into `specs/lmax-kubernetes/generation/runtime-overrides/...`, not into generated output alone.
...
| Stateful storage semantics | `009b` docs: journal dir, snapshot dir, projection checkpoint | Durable target: `specs/lmax-kubernetes/system/runtime-topology.md`, `requirements/nonfunctional-delta.md`, and Kubernetes manifest overrides | New Kubernetes design | Define PVCs or equivalent storage for journal/snapshot/checkpoint paths. This is not present as a durable manifest implementation in 009b because the original runtime was compose-oriented. |
| Observability and readiness | `009b` hot-path metrics + replay readiness semantics from `implementation-status.md` and `runtime-topology.md` | Durable target: `specs/lmax-kubernetes/generation/runtime-overrides/order-matcher/...` and manifest probes under Kubernetes/Tilt bases | Carry forward and expose | Keep LMAX metric families and ensure Kubernetes probes do not mark `order-matcher` healthy before replay/warm-up completes. |
| Price tick routing | `009b` runtime topology: price ticks reach Gateway for sequencing | Durable target: `specs/lmax-kubernetes/contracts/contract-delta.md`, `system/runtime-topology.md`, service/runtime overrides | Carry forward | `014` backend network shape otherwise stays intact; the meaningful backend route change is that price input must be treated as sequenced ingress, not out-of-band matcher subscription. |
...
1. Port `order-matcher` code/config overrides into `specs/lmax-kubernetes/generation/runtime-overrides/order-matcher/`.
