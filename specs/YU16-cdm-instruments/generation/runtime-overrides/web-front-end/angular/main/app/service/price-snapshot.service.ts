...
export class PriceSnapshotService {
  ...
  private normalizeSnapshot(snapshot: PriceTick | null | undefined, fallbackTicker?: string): PriceTick | null {
    ...
    // YU16 (FR-CDM19/20): the Treasury payload extensions ride through untouched — parsed,
    // never computed here (the publisher owns the YTM).
    const raw = snapshot as any;
    if (raw.assetClass === 'US_TREASURY') {
