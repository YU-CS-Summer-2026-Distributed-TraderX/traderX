...
export class PositionBlotterComponent implements OnChanges, OnDestroy {
  ...
  columnDefs: ColDef[] = [
    ...
    },
    {
      ...
      // YU16 (FR-CDM17/27): Treasury prices are fractions of par - display x100 with the sign.
      valueFormatter: ({ value, data }) => this.formatPriceFor(data?.security, value),
      headerClass: 'ag-right-aligned-header',
      cellClass: 'ag-right-aligned-cell'
    },
    {
    ...
    },
    {
      ...
      valueFormatter: ({ value, data }) =>
        String(data?.security || '').startsWith('UST-') && value != null
          ? this.formatPriceFor(data?.security, value)
          : this.formatMarketPrice(value, data?.openPrice)
    },
    {
  ...
  // YU16 (FR-CDM17): fraction x 100 with the sign for UST rows; currency otherwise.
  private formatPriceFor(security: string | undefined, value: unknown): string {
    if (String(security || '').startsWith('UST-') && value != null && Number.isFinite(Number(value))) {
