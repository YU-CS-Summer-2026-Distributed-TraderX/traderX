...
public final class OrderSnapshot {
    ...
    /** BLP authoritative decision (FR-IMRG15); ACCEPTED for every non-rejected lifecycle state. */
    public final RiskReason riskReason;
    ...
    private OrderSnapshot(int orderRef, String orderId, Integer accountId, String security, OrderSide side,
                          ...
                          RiskReason riskReason, Instant createdAt, Instant updatedAt,
    ...
    public static OrderSnapshot fromRecord(int orderRef, OrderRecord record) {
        return new OrderSnapshot(
            ...
            record.getStatus(),
            RiskReason.ACCEPTED,   // the DB read-model has no risk column; rejections live in the journal
            record.getCreatedAt(),
            record.getUpdatedAt(),
