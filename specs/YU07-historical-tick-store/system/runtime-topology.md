...
- `tick-store` gains its own PVC (`tick-store-data`) mounted at `/data/ticks`, separate from
  `order-matcher`'s `lmax-data` PVC — no shared volume with the hot path.
...
- `tick-store` connects directly to the existing `nats-broker` service — no new service-to-service
  HTTP calls.
- `tick-store` writes only to its own PVC; no other component reads or writes that volume.
...
| PVC full | The next Parquet flush fails and is logged; `capture.py` continues attempting subsequent flushes rather than crash-looping on one failed batch. |
