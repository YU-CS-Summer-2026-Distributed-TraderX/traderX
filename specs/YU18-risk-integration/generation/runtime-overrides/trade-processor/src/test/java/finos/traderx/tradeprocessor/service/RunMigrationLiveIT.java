...
@Tag("integration") @Testcontainers @Timeout(240)
@SpringBootTest(webEnvironment=SpringBootTest.WebEnvironment.RANDOM_PORT,properties={
 "spring.jpa.hibernate.ddl-auto=create","spring.sql.init.mode=never","RISK_CONTROL_TOKEN=ri06-proof-token",
 "recon.poll.interval-ms=3600000","order-matcher.base-url=http://127.0.0.1:1"})
class RunMigrationLiveIT {
 @Container static final MariaDBContainer<?> DB=new MariaDBContainer<>("mariadb:11.4")
  .withCommand("--lower_case_table_names=1").withCreateContainerCmdModifier(c->c.withName("traderx-ri06-live-sql").getHostConfig().withMemory(536870912L).withNanoCPUs(500000000L));
 @Container static final GenericContainer<?> NATS=new GenericContainer<>("nats:2.10-alpine").withExposedPorts(4222)
  .withCreateContainerCmdModifier(c->c.withName("traderx-ri06-live-nats").getHostConfig().withMemory(134217728L));
 ...
 @Autowired JdbcTemplate jdbc; @LocalServerPort int port;
 ...
 @Test void realOldAndFreshRunsMigrateThroughControllerAndResumeLostFreezeReply() throws Exception {
  work=Path.of(System.getenv().getOrDefault("RI06_LIVE_PROOF_DIR","build/ri06-live-proof")).toAbsolutePath();Files.createDirectories(work);
  try {
   ...
   var beforeTrades=jdbc.queryForList("SELECT * FROM trades ORDER BY id");var beforePositions=jdbc.queryForList("SELECT * FROM positions ORDER BY accountid,security");var beforeOrders=jdbc.queryForList("SELECT * FROM orderbook ORDER BY orderid");
   Map<String,String> request=Map.of("transitionId","live-migration","oldScope","legacy-unknown","descriptorJson",fresh,"oldEndpoint",oldGateway,"newEndpoint",freshGateway);
   assertEquals("PREPARED",post(controller(),"/v2/projection-control/prepare",request).path("phase").asText());seed(freshGateway);
   var early=http.send(HttpRequest.newBuilder(URI.create(freshGateway+"/run/control")).timeout(Duration.ofSeconds(10))
    .header("X-Risk-Control-Token",TOKEN).header("X-Risk-Operator","local-live-proof")
    .POST(HttpRequest.BodyPublishers.ofString(json.writeValueAsString(Map.of("operation","activate","descriptorHash",hash(fresh.getBytes(java.nio.charset.StandardCharsets.UTF_8)))))).build(),HttpResponse.BodyHandlers.ofString());
   assertEquals(409,early.statusCode());assertTrue(early.body().contains("RUN_ACTIVATION_NOT_VERIFIED"));
   assertEquals(0,jdbc.queryForObject("SELECT count(*) FROM trades WHERE projectionscope='fresh-live'",Integer.class));
   // Real controller commits freeze; a transport shim discards its reply to the caller.
   var proxy=HttpServer.create(new InetSocketAddress("127.0.0.1",0),0);
   proxy.createContext("/freeze",exchange->{try {post(controller(),"/v2/projection-control/freeze",request);}catch(Exception e){throw new RuntimeException(e);}finally{exchange.close();}});proxy.start();
   try {assertThrows(java.io.IOException.class,()->http.send(HttpRequest.newBuilder(URI.create("http://127.0.0.1:"+proxy.getAddress().getPort()+"/freeze")).POST(HttpRequest.BodyPublishers.ofString("{}" )).build(),HttpResponse.BodyHandlers.ofString()));} finally{proxy.stop(0);}
   assertEquals("FROZEN",jdbc.queryForObject("SELECT phase FROM projection_transitions WHERE transition_id='live-migration'",String.class));
   assertEquals("FROZEN",post(controller(),"/v2/projection-control/freeze",request).path("phase").asText());
   assertEquals("VERIFIED",post(controller(),"/v2/projection-control/verify",request).path("phase").asText());
   assertEquals("SELECTED",post(controller(),"/v2/projection-control/select",request).path("phase").asText());
   assertEquals("COMPLETE",post(controller(),"/v2/projection-control/activate",request).path("phase").asText());
   trade(freshGateway,20,"fresh-");
   ...
   assertEquals(beforeTrades,jdbc.queryForList("SELECT * FROM trades WHERE projectionscope='legacy-unknown' ORDER BY id"));
   assertEquals(beforePositions,jdbc.queryForList("SELECT * FROM positions WHERE projectionscope='legacy-unknown' ORDER BY accountid,security"));
   assertEquals(beforeOrders,jdbc.queryForList("SELECT * FROM orderbook WHERE projectionscope='legacy-unknown' ORDER BY orderid"));
   assertEquals(20,jdbc.queryForObject("SELECT quantity FROM positions WHERE projectionscope='fresh-live' AND accountid=11",Integer.class));
   assertEquals(-20,jdbc.queryForObject("SELECT quantity FROM positions WHERE projectionscope='fresh-live' AND accountid=12",Integer.class));
   assertEquals(List.of("freshlive-1","freshlive-2"),jdbc.queryForList("SELECT orderid FROM orderbook WHERE projectionscope='fresh-live' ORDER BY orderid",String.class));
   assertEquals(List.of("e1-freshlive-1-B","e1-freshlive-2-S"),jdbc.queryForList("SELECT id FROM trades WHERE projectionscope='fresh-live' ORDER BY id",String.class));
   System.out.println("RI06_LIVE_MIGRATION_WITNESS oldTrades=2 freshTrades=2 oldOrders=2 freshOrders=2 oldHistoryUnchanged=true lostFreezeReplyResumed=true phase=COMPLETE");
  } finally {
