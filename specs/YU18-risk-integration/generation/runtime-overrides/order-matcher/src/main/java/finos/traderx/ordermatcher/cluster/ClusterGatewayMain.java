...
public final class ClusterGatewayMain implements OrderSubmitter, OrderStatusSource {
    ...
    private static String reasonField(final ExecResult r) {
        // YU18: also on an ACCEPTED-kind answer that carries a reason — a FOK answered CANCELED
        // FOK_UNFILLABLE. An untyped accepted order always carries ACCEPTED (0), so is unchanged.
        if ((r.accepted() && r.riskReason() == 0) || r.riskReason() < 0
            || r.riskReason() >= RiskReason.values().length) {
            return "";
        }
    ...
    /** YU18 typed NEW (FR-OT01): same pipelined window and correlation, template 9 on the wire. */
    @Override
    public ExecResult submitTypedOrder(final String clOrdId, final int accountId, final String ticker,
        ...
        final PendingOrder p = new PendingOrder(InputEvent.TYPE_ORDER_NEW, accountId, ticker, side,
            qty, typed.limitPx(), clientOrderKey(clOrdId), 0);
        p.clOrdId = clOrdId;
        p.typed = typed;
        return submitPipelined(p);
    }

    /** YU18 typed REPLACE (FR-OT29). */
    @Override
    public ExecResult submitTypedReplace(final int orderRef, final String clOrdId, final int qty,
                                         final TypedOrder typed) {
        if (orderRef <= 0) {
            return new ExecResult(false, orderRef, OutputEvent.KIND_ORDER_NOT_FOUND);
        }
        final PendingOrder p = new PendingOrder(InputEvent.TYPE_ORDER_REPLACE, 0, null, (char) 0,
            qty, typed.limitPx(), clientOrderKey(clOrdId), orderRef);
        p.clOrdId = clOrdId;
        p.typed = typed;
        return submitPipelined(p);
    }
    ...
    private void offerPipelined(final PendingOrder p) {
        try {
            ...
            final int bytes;
            if (p.typed != null) {
                // YU18: typed NEW / REPLACE ride template 9; untyped keep template 1 byte for byte.
                event.orderType = p.typed.orderType();
                event.tif = p.typed.tif();
                event.stopPx = p.typed.stopPx();
                event.displayQty = p.typed.displayQty();
                event.pegRef = p.typed.pegRef();
                event.pegOffset = p.typed.pegOffset();
                event.trailMode = p.typed.trailMode();
                event.trailValue = p.typed.trailValue();
                event.admissionSeq = 0L;
                codec.encodeOrderInstruction(orderBuffer, 0, event, p.requestId, 0, 0);
                bytes = AeronReplicationCodec.ORDER_INSTRUCTION_BYTES;
            } else {
                event.clearOrderInstruction();
                codec.encodeInput(orderBuffer, 0, event, p.requestId, 0, 0);
                bytes = AeronReplicationCodec.INPUT_BYTES;
            }
            final long deadline = System.currentTimeMillis() + ACK_TIMEOUT_MS;
            pipelineOfferAttempts++;
            while (client.offer(orderBuffer, 0, bytes) < 0) {
    ...
    private void handleReplace(final HttpExchange exchange) {
        try {
            ...
            final ExecResult r;
            if (typedReplace) {
                // YU18 (FR-OT29): the side is the ORDER's, which only the engine holds; the side
                // is irrelevant to every stateless check except a peg offset's sign, which the
                // engine re-checks against the real side.
                final String sideName = body.path("side").asText("");
                final byte side = sideName.isEmpty()
                    ? (body.path("pegOffset").asInt(0) > 0 ? InputEvent.SIDE_SELL : InputEvent.SIDE_BUY)
                    : "Sell".equalsIgnoreCase(sideName) ? InputEvent.SIDE_SELL : InputEvent.SIDE_BUY;
                final Object parsed = parseTyped(body, side);
                if (parsed instanceof String error) {
                    respond(exchange, 422, "{\"error\":\"" + error + "\"}");
                    return;
                }
                r = submitTypedReplace(orderRef, body.path("clientOrderId").asText(""),
                    body.get("quantity").asInt(), (TypedOrder) parsed);
            } else {
                r = submitReplace(orderRef, body.path("clientOrderId").asText(""),
    ...
    private void handleOrder(final HttpExchange exchange) {
        try {
            ...
            // YU18 (FR-OT02/03): a typed request is validated in full HERE and nothing invalid is
            // sequenced; an untyped request keeps today's inference exactly, except that a
            // negative limitPrice and any order-type field without orderType are refused.
            final ExecResult r;
            if (body.has("orderType")) {
                final Object parsed = parseTyped(body, side == 'S' ? InputEvent.SIDE_SELL : InputEvent.SIDE_BUY);
                if (parsed instanceof String error) {
                    respond(exchange, 422, "{\"error\":\"" + error + "\"}");
                    return;
                }
                r = submitTypedOrder(body.path("clientOrderId").asText(""), body.path("accountId").asInt(),
                    ticker, side, qty, (TypedOrder) parsed);
            } else {
                final String legacyError = legacyOrderError(body, px);
                if (legacyError != null) {
                    respond(exchange, 422, "{\"error\":\"" + legacyError + "\"}");
                    return;
                }
                // Defaulted to "" rather than the old constant "rest": now that clOrdId feeds the
                // idempotency key, a shared default would make every key-less REST order a duplicate
                // of the first one. Empty means "no key", which is the pre-existing behaviour exactly.
                r = submitOrder(body.path("clientOrderId").asText(""),
    ...
    /**
     * YU18 (FR-OT37): POST /session/business-day {businessDate} = SESSION_START, POST
     * /session/day-end {businessDate} = DAY_END. The date is a calendar date, yyyy-MM-dd or
     * yyyyMMdd; the engine only compares the integer. 200 {outcome: APPLIED|STALE|OUT_OF_SEQUENCE}.
     */
    private void handleBusinessDay(final HttpExchange exchange, final boolean end) {
        try {
            if (!"POST".equals(exchange.getRequestMethod())) {
                respond(exchange, 405, "{\"error\":\"POST only\"}");
                return;
            ...
            final JsonNode body = JSON.readTree(exchange.getRequestBody());
            final int date = parseBusinessDate(body.path("businessDate").asText(""));
            if (date <= 0) {
                respond(exchange, 400, "{\"error\":\"businessDate must be a calendar date (yyyy-MM-dd)\"}");
                return;
            }
            final long requestId = clientOrderKey("business-day-" + date + "-" + end + "-" + System.nanoTime());
            final long[] ack = onOwner(() -> {
                event.type = InputEvent.TYPE_BUSINESS_DAY;
                event.side = end ? InputEvent.BUSINESS_DAY_END : InputEvent.BUSINESS_DAY_START;
                event.setClientOrderKey(requestId);
                event.accountId = 0;
                event.securityId = 0;
                event.orderRef = 0;
                ...
                event.clearOrderInstruction();
                codec.encodeInput(orderBuffer, 0, event, 0, 0, 0);
                lastBusinessDayAck = null;
                if (!offerAndAwait(orderBuffer, AeronReplicationCodec.INPUT_BYTES,
                        () -> lastBusinessDayAck != null && lastBusinessDayAck[2] == requestId)) {
                    return null;
                }
                return lastBusinessDayAck;
            });
            if (ack == null) {
                respond(exchange, 504, "{\"error\":\"no committed decision\"}");
                return;
            }
            final String[] outcomes = { "APPLIED", "STALE", "OUT_OF_SEQUENCE" };
            respond(exchange, 200, "{\"businessDate\":" + date + ",\"command\":\""
                + (end ? "DAY_END" : "SESSION_START") + "\",\"outcome\":\""
                + outcomes[(int) Math.max(0, Math.min(2, ack[1]))] + "\",\"sequence\":" + ack[0] + "}");
        } catch (final Exception e) {
            respond(exchange, 503, "{\"error\":\"" + e.getClass().getSimpleName() + "\"}");
        }
    }

    /** yyyy-MM-dd or yyyyMMdd to yyyymmdd; -1 unless it is a real calendar date. */
    static int parseBusinessDate(final String text) {
        try {
            final String digits = text.replace("-", "");
            if (digits.length() != 8) {
                return -1;
            }
            final java.time.LocalDate d = java.time.LocalDate.parse(digits,
    ...
    private static final String[] TYPED_ONLY_FIELDS = {
        "timeInForce", "stopPrice", "displayQuantity", "pegReference", "pegOffset", "trailAmount",
        "trailPercentBps"
    };

    /** FR-OT02: the untyped request's refusals; null = send it exactly as today. */
    static String legacyOrderError(final JsonNode body, final long limitPx) {
        if (limitPx < 0L) {
            return OrderTypes.reasonText(OrderTypes.LEGACY_NEGATIVE_PRICE);
        }
        for (final String field : TYPED_ONLY_FIELDS) {
            if (body.has(field)) {
                return field + " requires orderType";
            }
        }
        return null;
    }
    ...
    /**
     * Keep the wire decimal until typed validation: DoubleNode has already rounded it by the
     * time decimalValue() is called. Reparse only typed requests with an exact reader so legacy
     * requests retain their original Jackson node types and conversion behavior.
     */
    static JsonNode readOrderBody(final java.io.InputStream input) throws java.io.IOException {
        final byte[] bytes = input.readAllBytes();
        final JsonNode legacy = JSON.readTree(bytes);
        return legacy != null && legacy.has("orderType")
            ? JSON.reader().with(com.fasterxml.jackson.databind.DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS)
                .readTree(bytes)
            : legacy;
    }

    /**
     * A typed field's exact value x 10^places, or its 422 text as a {@code String}. The JSON type
     * and precision are judged BEFORE any coercion (review I3): {@code asInt}/{@code asLong} would
     * turn 1.9 bps into 1 and "10" into 10, and a sentinel zero would hide a field that was sent.
     */
    private static Object exactField(final JsonNode body, final String field, final int places) {
        final JsonNode n = body.get(field);
        if (n == null || !n.isNumber()) {
            return field + " must be a number";
        }
        final long v = OrderTypes.exactScaled(n.decimalValue(), places);
        if (v == OrderTypes.NOT_EXACT) {
            return places == 0 ? field + " must be a whole number"
                : field + " allows at most " + places + " decimal places";
        }
        return v;
    }

    /** Price-unit fields carry 6 decimals (the 1e6 tick scale); counts, bps and peg ticks none. */
    private static final int PRICE_PLACES = 6;

    /**
     * FR-OT01/03/41: a typed REST body to a {@link TypedOrder}, or the 422 message. Presence and
     * JSON types are checked here, exactly; every value rule is then {@link OrderTypes#validate},
     * the table FIX and the console share. {@code quantity} is checked too, so a caller's
     * {@code asInt()} of it is exact once this returns a TypedOrder.
     */
    static Object parseTyped(final JsonNode body, final byte side) {
        final JsonNode typeNode = body.get("orderType");
        final byte type = typeNode != null && typeNode.isTextual()
            ? OrderTypes.parseType(typeNode.asText()) : (byte) -1;
        if (type < 0) {
            return OrderTypes.reasonText(OrderTypes.BAD_TYPE);
        }
        final Object q = exactField(body, "quantity", 0);
        if (q instanceof String) {
            return body.has("quantity") ? q : OrderTypes.reasonText(OrderTypes.BAD_QUANTITY);
        }
        if ((Long) q <= 0L || (Long) q > Integer.MAX_VALUE) {
            return OrderTypes.reasonText(OrderTypes.BAD_QUANTITY);   // saturation must not pass
        }
        final int qty = (int) (long) (Long) q;
        byte tif = OrderTypes.defaultTif(type);
        if (body.has("timeInForce")) {
            final JsonNode t = body.get("timeInForce");
            tif = t.isTextual() ? OrderTypes.parseTif(t.asText()) : (byte) -1;
            if (tif < 0) {
                return OrderTypes.reasonText(OrderTypes.BAD_TIF);
            }
        }
        final int present = (body.has("limitPrice") ? OrderTypes.F_LIMIT : 0)
            | (body.has("stopPrice") ? OrderTypes.F_STOP : 0)
            | (body.has("displayQuantity") ? OrderTypes.F_DISPLAY : 0)
            | (body.has("pegReference") || body.has("pegOffset") ? OrderTypes.F_PEG : 0)
            | (body.has("trailAmount") || body.has("trailPercentBps") ? OrderTypes.F_TRAIL : 0);
        if (!OrderTypes.fieldsAllowed(type, present)) {
            return OrderTypes.reasonText(OrderTypes.FIELD_NOT_ALLOWED);
        }
        byte pegRef = 0;
        if (body.has("pegReference")) {
            final JsonNode r = body.get("pegReference");
            final String ref = r.isTextual() ? r.asText().toUpperCase(java.util.Locale.ROOT) : "";
            pegRef = "PRIMARY".equals(ref) ? OrderTypes.PEG_PRIMARY
                : "MIDPOINT".equals(ref) ? OrderTypes.PEG_MIDPOINT : (byte) -1;
            if (pegRef < 0) {
                return OrderTypes.reasonText(OrderTypes.BAD_PEG_REFERENCE);
            }
        }
        final long[] v = new long[6];   // limit, stop, display, pegOffset, trailAmount, trailBps
        final String[] fields = { "limitPrice", "stopPrice", "displayQuantity", "pegOffset",
            "trailAmount", "trailPercentBps" };
        final int[] places = { PRICE_PLACES, PRICE_PLACES, 0, 0, PRICE_PLACES, 0 };
        for (int i = 0; i < fields.length; i++) {
            if (body.has(fields[i])) {
                final Object x = exactField(body, fields[i], places[i]);
                if (x instanceof String error) {
                    return error;
                }
                v[i] = (Long) x;
            }
        }
        // A price that was SENT as 0 is refused as a bad price, not reported missing: -1 carries
        // "present but not positive" through validate's zero-means-absent convention.
        final long limitPx = body.has("limitPrice") && v[0] == 0L ? -1L : v[0];
        final long stopPx = body.has("stopPrice") && v[1] == 0L ? -1L : v[1];
        final int displayQty = OrderTypes.saturatedInt(v[2]);
        final int pegOffset = OrderTypes.saturatedInt(v[3]);
        final boolean amount = body.has("trailAmount");
        final boolean bps = body.has("trailPercentBps");
        if (amount && bps) {
            return OrderTypes.reasonText(OrderTypes.BAD_TRAIL);
        }
        final byte trailMode = amount ? OrderTypes.TRAIL_AMOUNT : bps ? OrderTypes.TRAIL_BPS : 0;
        final long trailValue = amount ? v[4] : v[5];
        final int code = OrderTypes.validate(type, tif, side, qty, limitPx, stopPx, displayQty, pegRef,
            pegOffset, trailMode, trailValue);
        if (code != OrderTypes.OK) {
            return OrderTypes.reasonText(code);
        }
        return new TypedOrder(type, tif, limitPx, stopPx, displayQty, pegRef, pegOffset, trailMode, trailValue);
    }

    private void handleSession(final HttpExchange exchange) {
        final String path = exchange.getRequestURI().getPath();
        if (path.endsWith("/business-day") || path.endsWith("/day-end")) {
            handleBusinessDay(exchange, path.endsWith("/day-end"));   // YU18 FR-OT37, same route guard
            return;
        }
        try {
            if (!"POST".equals(exchange.getRequestMethod())) {
    ...
    static final class PendingOrder {
        ...
        /** YU18: non-null for a typed NEW / REPLACE, which is encoded as SBE template 9. */
        OrderSubmitter.TypedOrder typed;
