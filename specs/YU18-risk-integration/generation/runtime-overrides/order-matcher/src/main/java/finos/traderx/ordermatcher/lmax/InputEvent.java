...
public final class InputEvent {
    ...
    /**
     * YU18 order types (FR-OT37): the business-date lifecycle, sequenced like every control. A new
     * command type on the EXISTING template, no schema change. Payload: {@code side} =
     * {@link #BUSINESS_DAY_START} or {@link #BUSINESS_DAY_END}; {@code qty} = the date as an
     * integer yyyymmdd (the gateway validates it is a calendar date; the engine only compares);
     ...
     *
     * <p>ADR-072 requires replayed flow to be "attributable and excludable at the source" — a
     * dedicated account, or an equivalent tag carried into consensus. {@code accountId} is already
     * on this event and already sequenced, so the account range IS the tag and it costs no new
     * command type, no new wire field and nothing that could disagree per member.
     *
     * <p>A COMPILE-TIME CONSTANT, deliberately, and not configuration. Four replicated counters
     * branch on it ({@code MatchingEngineClusteredService.externalOrderRefs},
     * {@code MatchingEngine.externalTradeLegs} and the two band shadows); a member holding a
     * different value for it would compute a different function of the same committed log, which
     * is divergence rather than a misconfiguration. The seeded fixture accounts are all five
     * digits, so the range is disjoint from every real account by construction — and
     * print-replay.js refuses to start on an account below it, so the two ends cannot drift.
     */
    public static final int REPLAY_ACCOUNT_BASE = 900_000;

    /** True for order flow generated by the ADR-072 tape replay rather than by an operator. */
    public static boolean isReplayFlow(final int accountId) {
        return accountId >= REPLAY_ACCOUNT_BASE;
    }
    ...
    // ----- YU18 typed order instruction (FR-OT01, FR-OT36) -------------------------------------
    // Carried ONLY by SBE template 9 (OrderInstructionMessage). Every decode of the legacy template
    // calls clearOrderInstruction(), so a reused slot can never leak a previous typed order's
    // fields into an untyped one, and old journals replay as orderType LEGACY.
    public byte orderType;        // OrderTypes.*; LEGACY (0) = untyped request, inferred from price
    public byte tif;              // OrderTypes.TIF_*; resolved at the gateway for typed orders
    public byte pegRef;           // OrderTypes.PEG_*
    public byte trailMode;        // OrderTypes.TRAIL_*
    public int displayQty;        // ICEBERG display quantity
    public int pegOffset;         // PEGGED offset in grid ticks (signed)
    public long stopPx;           // STOP / STOP_LIMIT stop price
    public long trailValue;       // TRAILING_STOP amount (Px) or basis points
    /**
     * Admission sequence override (FR-OT11): the log sequence of the command that SUBMITTED the
     * order. 0 = this command's own sequence. Only the PRE_OPEN release sets it, to the sequence
     * the order was queued under, so orders released together keep their original order.
     */
    ...
    public void clearOrderInstruction() {
        orderType = 0;
        tif = 0;
        pegRef = 0;
        trailMode = 0;
