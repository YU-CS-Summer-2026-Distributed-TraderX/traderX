...
import finos.traderx.ordermatcher.replication.sbe.OrderInstructionMessageDecoder;
import finos.traderx.ordermatcher.replication.sbe.OrderInstructionMessageEncoder;
...
public final class AeronReplicationCodec {
    ...
    /** YU18 order types: a typed ORDER_NEW / ORDER_REPLACE (template 9). */
    public static final int ORDER_INSTRUCTION_BYTES = MessageHeaderEncoder.ENCODED_LENGTH
        + OrderInstructionMessageEncoder.BLOCK_LENGTH;
    public static final int ORDER_INSTRUCTION_TEMPLATE_ID = OrderInstructionMessageDecoder.TEMPLATE_ID;
    ...
    private final OrderInstructionMessageEncoder instructionEncoder = new OrderInstructionMessageEncoder();
    private final OrderInstructionMessageDecoder instructionDecoder = new OrderInstructionMessageDecoder();
    ...
    public void decodeInspectedInput(InputEvent target) {
        ...
        target.clearOrderInstruction();   // YU18: template 1 is always an UNTYPED command
    }

    /**
     * YU18: encode a typed ORDER_NEW / ORDER_REPLACE as template 9. The first 56 bytes are laid out
     * exactly as template 1, so the member's ingress handling (request id in inputSeq, the
     * sequenced generator's orderRef overwrite) is the same code path.
     */
    public void encodeOrderInstruction(MutableDirectBuffer buffer, int offset, InputEvent event,
                                       long primaryInputSeq, long epoch, int flags) {
        instructionEncoder.wrapAndApplyHeader(buffer, offset, headerEncoder)
            .inputSeq(primaryInputSeq)
            .eventTimeMillis(event.eventTimeMillis)
            .limitPx(event.limitPx)
            .priceTicks(event.priceTicks)
            ...
            .side((short) Byte.toUnsignedInt(event.side))
            .flags(flags)
            .orderType((short) Byte.toUnsignedInt(event.orderType))
            .timeInForce((short) Byte.toUnsignedInt(event.tif))
            .pegReference((short) Byte.toUnsignedInt(event.pegRef))
            .trailMode((short) Byte.toUnsignedInt(event.trailMode))
            .displayQty(event.displayQty)
            .pegOffset(event.pegOffset)
            .reserved(0)
            .stopPx(event.stopPx)
            .trailValue(event.trailValue);
    }

    /** YU18: decode template 9 into {@code target}; fails closed exactly as template 1 does. */
    public int tryDecodeOrderInstruction(DirectBuffer buffer, int offset, int length, InputEvent target) {
        final int status = validateHeader(buffer, offset, length, ORDER_INSTRUCTION_BYTES,
            OrderInstructionMessageDecoder.TEMPLATE_ID, OrderInstructionMessageDecoder.BLOCK_LENGTH,
            OrderInstructionMessageDecoder.SCHEMA_VERSION);
        if (status != OK) return status;
        instructionDecoder.wrap(buffer, offset + MessageHeaderDecoder.ENCODED_LENGTH,
            headerDecoder.blockLength(), headerDecoder.version());
        final int flags = instructionDecoder.flags();
        if ((flags & ~KNOWN_INPUT_FLAGS) != 0) return UNKNOWN_FLAGS;
        inputSeq = instructionDecoder.inputSeq();
        leaderEpoch = instructionDecoder.leaderEpoch();
        inputFlags = flags;
        target.seq = inputSeq;
        target.eventTimeMillis = instructionDecoder.eventTimeMillis();
        target.limitPx = instructionDecoder.limitPx();
        target.priceTicks = instructionDecoder.priceTicks();
        target.orderRef = (int) instructionDecoder.orderRef();
        target.accountId = (int) instructionDecoder.accountId();
        target.securityId = (int) instructionDecoder.securityId();
        target.qty = instructionDecoder.qty();
        target.type = (byte) instructionDecoder.commandType();
        target.side = (byte) instructionDecoder.side();
        target.ingressNanos = 0L;
        target.orderType = (byte) instructionDecoder.orderType();
        target.tif = (byte) instructionDecoder.timeInForce();
        target.pegRef = (byte) instructionDecoder.pegReference();
        target.trailMode = (byte) instructionDecoder.trailMode();
        target.displayQty = instructionDecoder.displayQty();
        target.pegOffset = instructionDecoder.pegOffset();
        target.stopPx = instructionDecoder.stopPx();
        target.trailValue = instructionDecoder.trailValue();
        target.admissionSeq = 0L;
        return OK;
    }
