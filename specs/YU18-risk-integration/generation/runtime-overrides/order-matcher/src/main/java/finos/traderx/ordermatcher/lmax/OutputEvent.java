...
public final class OutputEvent {
    ...
    /** YU18 (FR-OT27): this update is a STOP/STOP_LIMIT/TRAILING_STOP's trigger conversion. */
    public static final int FLAG_TRIGGERED = 1 << 8;
    ...
    // YU18 order types: the order's typed shape, copied from the order on every TYPED order update
    // so the read model can show it (FR-OT33/34). orderType stays inline (LEGACY = 0 means every
    // shape field is zero); the rest lives in a side object made once per ring slot. NFR-OT06
    // (review I4): the ring cycles every slot through cache, and 40-odd more bytes inline is one
    // more line per legacy event, measured on the legacy match path.
    public byte orderType;
    public final TypedShape typed = new TypedShape();

    /** The typed fields of an order update; all zero while {@link #orderType} is LEGACY. */
    public static final class TypedShape {
        public byte tif;
        public long stopPx;
