...
public final class MatchingEngineClusteredService implements ClusteredService {
    ...
    // Format 11 (YU18 order types, FR-OT35): T_ORDER_EXT before each typed order row, open rows in
    ...
    /** YU18 format 11: typed state of the T_ORDER row that FOLLOWS it (MatchingEngine.ORDER_EXT_TUPLE_LENGTH). */
    static final int T_ORDER_EXT = 17;
    /** YU18 format 11: {securityId, lastTradePx, hasTraded, pegRefBid, pegRefAsk}. */
    ...
    static final int QUEUED_TUPLE_LENGTH = 17;
    /** Formats 9 and 10 wrote the first eight columns only. YU18 appends {orderType, tif, stopPx,
     *  displayQty, pegRef, pegOffset, trailMode, trailValue, admissionSeq}: the typed instruction
     *  and the sequence the order was SUBMITTED under (FR-OT11 tie-breaker at the open). */
    static final int LEGACY_QUEUED_TUPLE_LENGTH = 8;
    ...
    /** YU18 (FR-OT37): SESSION_START / DAY_END ack. Byte 8 = outcome (0 applied, 1 stale, 2 out
     *  of sequence), bytes 13..20 = the issuer's correlation id, 24..31 = 0. Every egress kind is
    ...
    /** The format of the snapshot being restored (formats 9..11 are readable). */
    private int restoredFormat;
    ...
    public void onSessionMessage(final ClientSession session, final long timestamp,
        ...
        if (templateId == AeronReplicationCodec.ORDER_INSTRUCTION_TEMPLATE_ID) {
            // YU18: a typed ORDER_NEW / ORDER_REPLACE. Anything else on template 9 fails closed.
            if (codec.tryDecodeOrderInstruction(buffer, offset, length, event) != AeronReplicationCodec.OK
                || (event.type != InputEvent.TYPE_ORDER_NEW && event.type != InputEvent.TYPE_ORDER_REPLACE)) {
                return;
            }
        } else if (codec.tryDecodeInput(buffer, offset, length, event) != AeronReplicationCodec.OK) {
        ...
        if (event.type == InputEvent.TYPE_BUSINESS_DAY) {
            // YU18 (FR-OT37): the business date, sequenced here in EVERY phase — a halt never
            // blocks or causes day expiry. Emits DAY_EXPIRED order egress, so it drains like
            // the session control below.
            onBusinessDay(session, timestamp);
            return;
        }
    ...
    private void onSandboxReset(final ClientSession session, final long timestamp) {
        ...
        final int keptBusinessDate = engine.businessDate();   // YU18: the date is VENUE state
        final boolean keptDayOpen = engine.dayOpen();
        ...
        phase = keptPhase;
        engine.bootstrapBusinessDay(keptBusinessDate, keptDayOpen);
    ...
    /**
     * YU18 (FR-OT08, FR-OT37): SESSION_START / DAY_END. The engine owns the date and expires the
     * book's DAY orders (resting, pending, suspended) in ascending orderRef; this service then
     * removes the PRE_OPEN queue's DAY entries, in queue order, with the same reason. Applied in
     * every phase; the halt and the trading day are orthogonal.
     */
    private void onBusinessDay(final ClientSession session, final long timestamp) {
        event.seq = ++appliedSeq;
        final boolean nanosClusterClock = cluster != null && cluster.timeUnit() == TimeUnit.NANOSECONDS;
        event.eventTimeMillis = nanosClusterClock ? timestamp / 1_000_000L : timestamp;
        event.ingressNanos = System.nanoTime();   // telemetry only, never state
        activeSession = session;
        applyRequestId = 0L;   // expiry egress completes no pending; the ack below correlates
        engine.onEvent(event, appliedSeq, true);
        final byte outcome = engine.lastBusinessDayOutcome();
        if (outcome == MatchingEngine.DAY_APPLIED) {
            for (int i = 0; i < queuedOrders.size(); ) {
                final long[] row = queuedOrders.get(i);
                if (row[9] == finos.traderx.ordermatcher.lmax.OrderTypes.DAY) {
                    queuedOrders.remove(i);
                    emitQueuedTerminal(row, RiskReason.DAY_EXPIRED);
                } else {
                    i++;
                }
            }
        ...
        ackBuffer.putLong(0, appliedSeq);
        ackBuffer.putInt(8, outcome);
        ackBuffer.putByte(12, KIND_BUSINESS_DAY);
        ackBuffer.putLong(13, event.clientOrderKey());
        ackBuffer.putByte(21, (byte) 0);
        ackBuffer.putByte(22, (byte) 0);
        ackBuffer.putByte(23, (byte) 0);
    ...
    private void queueOrder() {
        ...
        if (event.tif == finos.traderx.ordermatcher.lmax.OrderTypes.DAY && !engine.dayOpen()) {
            refuseNewOrder(RiskReason.NO_TRADING_DAY);   // YU18 FR-OT08: queue time is admission
            return;
        }
        final long[] row = new long[] {
            ...
            event.orderType, event.tif, event.stopPx, event.displayQty, event.pegRef,
            event.pegOffset, event.trailMode, event.trailValue, event.seq };
        if (clientKey != 0L) {
    ...
    private void releaseQueue() {
        ...
        for (final long[] row : released) {
            ...
            // YU18: the typed instruction, and the sequence it was SUBMITTED under — orders released
            // together keep their original order in every same-print tie (FR-OT11).
            releaseEvent.orderType = (byte) row[8];
            releaseEvent.tif = (byte) row[9];
            releaseEvent.stopPx = row[10];
            releaseEvent.displayQty = (int) row[11];
            releaseEvent.pegRef = (byte) row[12];
            releaseEvent.pegOffset = (int) row[13];
            releaseEvent.trailMode = (byte) row[14];
            releaseEvent.trailValue = row[15];
            releaseEvent.admissionSeq = row[16];
    ...
    private void loadSessionOrder(final long[] row) {
        ...
        // YU18: a queued typed order shows its type in the read model while it waits.
        sessionOrder.orderType = (byte) row[8];
        sessionOrder.tif = (byte) row[9];
        sessionOrder.stopPx = row[10];
        sessionOrder.displayQty = (int) row[11];
        sessionOrder.pegRef = (byte) row[12];
        sessionOrder.pegOffset = (int) row[13];
        sessionOrder.trailMode = (byte) row[14];
        sessionOrder.trailValue = row[15];
        if (sessionOrder.orderType == finos.traderx.ordermatcher.lmax.OrderTypes.PEGGED) {
            sessionOrder.capPx = row[5];
        }
    }
    ...
    void writeSnapshot(final SnapshotWriter writer) {
        ...
        // YU18 format 11: the business date and the consensus limits in force (review r3 item 1),
        // always written. A restore under different limits is refused: FR-OT40's outcome depends
        // on them, so two members holding different ones would diverge on the same log.
        if (engine.triggerQueueDepth() != 0) {
            throw new IllegalStateException("snapshot refused: trigger queue not empty at a boundary");
        }
        final long[] limits = engine.orderTypeLimits();
        writeTuple(writer, T_ORDER_TYPES, new long[] { engine.businessDate(), engine.dayOpen() ? 1L : 0L,
            limits[0], limits[1], limits[2] });
        for (final long[] sec : engine.orderTypeSecurityTuples()) {
            writeTuple(writer, T_OT_SECURITY, sec);
        }
        ...
        // YU18 format 11: open rows in BOOK-APPEND order (every level's exact FIFO, including orders
        // that replenished, repriced or replaced to a tail), then held open orders (pending,
        // suspended); a typed order's T_ORDER_EXT immediately precedes its row.
        final long[] extTuple = new long[MatchingEngine.ORDER_EXT_TUPLE_LENGTH];
        for (final int ref : engine.snapshotOpenRefsInRestoreOrder()) {
            if (!terminalSet.contains(ref) && engine.copySnapshotOrderTuple(ref, orderTuple)) {
                if (engine.copyOrderExtension(ref, extTuple)) {
                    writeTuple(writer, T_ORDER_EXT, extTuple);
                }
    ...
    boolean onSnapshotRecord(final DirectBuffer buffer, final int offset) {
        ...
        switch (type) {
            ...
            case T_QUEUED_ORDER -> {
                // Formats 9-10 wrote eight columns; the typed columns then read as 0 (untyped).
                final long[] queued = new long[QUEUED_TUPLE_LENGTH];
                final int columns = restoredFormat >= 11 ? QUEUED_TUPLE_LENGTH : LEGACY_QUEUED_TUPLE_LENGTH;
            ...
            case T_ORDER_EXT -> {
                final long[] ext = new long[MatchingEngine.ORDER_EXT_TUPLE_LENGTH];
                for (int i = 0; i < ext.length; i++) {
                    ext[i] = buffer.getLong(offset + 4 + 8 * i);
                }
                engine.stageOrderExtension(ext);
            }
            case T_OT_SECURITY -> {
                final int s = (int) buffer.getLong(offset + 4);
                if (s < 0 || s >= MAX_SECURITIES) {
                    throw new IllegalStateException("snapshot corrupt: order-type security " + s);
                }
                engine.bootstrapOrderTypeSecurity(s, buffer.getLong(offset + 12),
                    buffer.getLong(offset + 20) != 0L, buffer.getLong(offset + 28), buffer.getLong(offset + 36));
            }
            case T_ORDER_TYPES -> {
                final long[] mine = engine.orderTypeLimits();
                final long pending = buffer.getLong(offset + 20);
                final long pegs = buffer.getLong(offset + 28);
                final long rounds = buffer.getLong(offset + 36);
                if (pending != mine[0] || pegs != mine[1] || rounds != mine[2]) {
                    // Review r3 item 1: consensus limits. Restoring under different ones would let
                    // this member compute a different FR-OT40 outcome from the same log.
                    throw new IllegalStateException("snapshot refused: order-type limits {" + pending
                        + "," + pegs + "," + rounds + "} differ from this member's {" + mine[0] + ","
                        + mine[1] + "," + mine[2] + "}");
                }
    ...
    void finishLoad() {
        ...
        // YU18 format 11, checked after every inherited invariant so those keep their own refusal.
        if (restoredFormat >= 11 && (recordTypesSeen & (1 << T_ORDER_TYPES)) == 0) {
