...
public final class LimitBook {
    ...
    // YU17 (format-8 price-derived grid, design section 2.3): NOT final. A book with no resting
    // orders re-derives its grid from the collar's replicated reference at the next admission, so
    // the tick is per-occupancy state rather than a construction-time constant. It never changes
    // while an order rests -- see retick(), which throws if one does.
    private long tickTicks;
    ...
    /**
     * YU17 (format-8 design section 2.3): move an EMPTY book onto a new grid and un-anchor it, so
     * the next limit re-anchors the band on the reference at the new scale.
     *
     * <p>Throws on an occupied book, deliberately and unconditionally: every resting order's
     * {@code bookLevel} is an index into a band denominated in the OLD tick, so a retick with
     * orders in it would silently reinterpret every one of them at a different scale. The engine
     * only ever calls this when {@code openOrders() == 0}, so a throw here is a caller bug, not a
     * data condition. An empty book needs no array work: {@code remove} clears each level's head,
     * tail, aggregate quantity and occupancy bit as it empties it, and the best pointers follow
     * (see LimitBookRetickTest, implementation gate V1).
     */
    public void retick(long newTickTicks) {
        if (openOrders != 0) {
            throw new IllegalStateException("retick of an occupied book");
        }
        if (newTickTicks <= 0) {
            throw new IllegalArgumentException("book tick must be positive");
        }
        this.tickTicks = newTickTicks;
        this.baseLevel = -1;   // un-anchored: bandSlot re-centres on the reference at the new scale
    }
    ...
    public void append(RestingOrder o, int slot) {
        ...
        tail[slot] = o;
        // YU18 (FR-OT20): the level shows only an iceberg's tranche; hidden quantity is off-book.
        (bid ? bidQty : askQty)[slot] += o.visibleQty();
        openOrders++;
    }
     ...
     * (an iceberg's {@code displayed}) still holds the order's visible quantity — the level
     * aggregate subtracts it.
     */
