...
final class ClusterRecon {
    ...
    void runDescriptor(RunDescriptor descriptor) {this.runDescriptor=descriptor;}
    ...
    synchronized java.util.Map<String,Object> projectionEvents(boolean strict) {
        ...
        long[] input={-1};int[] ordinal={0};long[] applied={0,0};
        long trades=replay((out,ticker)->{
            if(input[0]!=out.inputSeq) {input[0]=out.inputSeq;ordinal[0]=0;}
            int index=ordinal[0]++;
            byte[] wire=null;
            if(out.kind==OutputEvent.KIND_TRADE_BOOKED) {
            ...
            if(wire!=null) {
                if(events.size()>=regulatoryMax) {throw new IllegalStateException("RUN_REPLAY_LIMIT_EXCEEDED");}
                try {var envelope=mapper.readTree(wire);
    ...
    private TradeBlotter.TradeRecord tradeRecord(final OutputEvent out, final String ticker) {
        ...
        return new TradeBlotter.TradeRecord(
            runDescriptor == null ? out.tradeSeq + (buy ? "-B" : "-S")
                : runDescriptor.tradeId(out.tradeSeq, out.side),
            out.tradeSeq,
            out.accountId,
            ticker,
    ...
    private AuditRow auditRow(final OutputEvent out, final String ticker) {
        ...
        return new AuditRow(
            ...
            runDescriptor == null ? epoch + "-" + out.orderRef : runDescriptor.orderId(out.orderRef),
            trade ? (runDescriptor==null ? out.tradeSeq + (buy ? "-B" : "-S")
                : runDescriptor.tradeId(out.tradeSeq,out.side)) : null,
            out.accountId,
            ticker,
            OrderSide.values()[out.side].name(),
