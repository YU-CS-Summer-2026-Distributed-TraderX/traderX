...
public final class ClusterNodeMain {
    public static void main(final String[] args) throws Exception {
        ...
        final RunDescriptor descriptor = RunDescriptor.load(baseDir.toPath(),
            System.getenv("RUN_DESCRIPTOR_PATH"), System.getenv("CLUSTER_EPOCH"));
        final MatchingEngineClusteredService service = new MatchingEngineClusteredService();
        service.runDescriptor(descriptor);
        ...
        if (recon!=null) {recon.runDescriptor(descriptor);}
        if (recon != null) {
    ...
    private static HttpServer healthServer(final int port, final int memberId,
        ...
        server.createContext("/health", exchange -> {
            ...
            final String body = "{\"memberId\":" + memberId
                ...
                + ",\"runProtocol\":1,\"runPhase\":" + service.runPhase()
                + ",\"frozenRunSequence\":" + service.frozenRunSequence()
                + ",\"runDescriptorHash\":" + (service.runDescriptor()==null ? "null" : "\""+service.runDescriptor().hash()+"\"")
                + ",\"projectionScope\":" + (service.runDescriptor()==null ? "null" : "\""+service.runDescriptor().projectionScope()+"\"")
    ...
    private static void reconRoutes(final HttpServer server,
        ...
        server.createContext("/recon/projection-events", exchange -> guarded.apply(exchange,()->{
            ...
            var result=new java.util.HashMap<String,Object>(recon.projectionEvents());
            if(((Number)result.get("shadowTradeCounter")).longValue()!=service.engine().tradeCounter()
                || ((Number)result.get("replayedAppliedSeq")).longValue()<service.frozenRunSequence()) {
                throw new IllegalStateException("RUN_REPLAY_INCOMPLETE_AT_FROZEN_BOUNDARY");
            }
            result.put("frozenSequence",service.frozenRunSequence());return result;
        }));
