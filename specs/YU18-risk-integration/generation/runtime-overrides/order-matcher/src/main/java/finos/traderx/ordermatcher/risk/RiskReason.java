...
public enum RiskReason {
     ...
     */
    SESSION_CANCELED,
    // YU18 order types (components/order-types, FR-OT13..41). Appended, never inserted: the
    // ordinal rides every snapshot order row and every egress ack byte 22.
    /** FR-OT13: a STOP/STOP_LIMIT whose stop is already through the last trade at admission. */
    STOP_ALREADY_TRIGGERED,
    /** FR-OT18: a TRAILING_STOP admitted before this security has traded. */
    TRAIL_REFERENCE_MISSING,
    /** FR-OT41: the derived trailing stop is not in (0, MAX_PRICE_TICKS]. */
    TRAIL_INVALID,
    /** FR-OT24: the local-book peg reference is absent at admission. */
    PEG_REFERENCE_MISSING,
    /** FR-OT41: the derived peg price is not in (0, MAX_PRICE_TICKS]. */
    PEG_PRICE_INVALID,
    /** FR-OT17/31: a FOK the book cannot fill in full; rides a CANCELED order. */
    FOK_UNFILLABLE,
    /** FR-OT08: a DAY order expired by the end of its business date; rides a CANCELED order. */
    DAY_EXPIRED,
    /** FR-OT08: a DAY order submitted while no trading day is open. */
    NO_TRADING_DAY,
    /** FR-OT40: cancelled by the cascade exhaustion path; rides a CANCELED order. */
    CASCADE_LIMIT
}
