...
Run `bash scripts/test-state-YU18-risk-integration.sh` from the repository root. The suite validates synthetic source exports, builds a private temporary bundle, validates it through the CLI, and invokes the mock consumer. A shell trap removes only the test-created temporary directory.
