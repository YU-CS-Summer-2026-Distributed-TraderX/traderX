  ...
  template: `
    ...
    @if (!api.adminToken()) {
      ...
      <div class="bar">
        ...
        <span class="faint">token: {{ tokenSubject() }} <a (click)="api.dropAdminToken()">forget</a></span>
      </div>
      @if (report() && !isLatest()) {
...
export class EodPanel {
  ...
  readonly tokenSubject = computed(() => {
    try { return JSON.parse(atob(this.api.adminToken()!.split('.')[1])).sub ?? '?'; } catch { return '?'; }
  });
  ...
  constructor() {
    ...
    if (!this.api.adminToken()) {
      ...
      this.api.mintAdminToken().then(ok => { this.minting.set(false); if (ok) this.loadLatest(); });
    } else {
      this.loadLatest();
    }
  }
  ...
  async mint(): Promise<void> {
    const ok = await this.api.mintAdminToken(this.masterSecret);
    if (ok) { this.authError.set(''); this.masterSecret = ''; this.loadLatest(); }
    else this.authError.set('invalid master secret');
  }

  async loadLatest(): Promise<void> {
    const r = await this.api.load<EodReport>(`/trade-processor/eod/prices/${this.date}`, { headers: this.api.authHeaders() });
  ...
  async loadVersion(v: number): Promise<void> {
    const r = await this.api.load<EodReport>(`/trade-processor/eod/prices/${this.date}/versions/${v}`, { headers: this.api.authHeaders() });
  ...
  async applyOverride(): Promise<void> {
    ...
    const r = await this.api.load<EodReport>(`/trade-processor/eod/prices/${this.date}/override`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...this.api.authHeaders() },
