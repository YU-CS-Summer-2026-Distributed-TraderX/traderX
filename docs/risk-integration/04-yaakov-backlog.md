...
- [x] **Y-U08 · RI-06/07 — Managed-run blotter context.** Active-run context, read-only named-run history for positions/trades/orders, run-scoped notifications and stale-reply fencing. Implemented and locally proven 2026-09-24 on a disposable fixture rig (review needed); see [managed-run UI](managed-run-ui.md). Active pointer reader integrated (3f8db417) and adopted in corrections R1–R3; Admin trade list run-aware; console proven across a real local RI-06 transition (review needed). Open: rollout on a managed installation once deployment is authorized.
