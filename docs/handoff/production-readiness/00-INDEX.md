...
| [03](03-baseline-unit-tests.md) → [RESULT](03-RESULT-baseline-unit-tests.md) | Unit tests for the plain-vanilla TraderX baseline — **✅ DONE 2026-07-25** (`2759cfd6`, `ed78fbac`). **0 → 25 executable tests** across the 4 inherited Java services (account 8 / trade 5 / position 5 / processor 7); root cause = the `src/main/test/java` NO-SOURCE silent-pass trap; new `baseline` CI job. Deferred to brief 04: TS/.NET services + per-state composed-tree matrix. | done (needs push to run in CI) |
| [04](04-milestone-and-integration-tests.md) → [CI RESULT](04-RESULT-ci-bringup.md) | **CI wiring ✅ DONE + GREEN** (`4abcfdf2`, badge live, YU15 hosted 298 tests + 4 gates, 3m49s). Remaining: milestone/integration tests, promote 17 proof scripts to CI, extend matrix to YU13/14. **⚠️ CI radar (surfaced by the rebase, pre-existing): the earlier states' order-matcher has ~12 `@SpringBootTest` context-load tests (OrderMatcherApplicationTests, LmaxHotPathParityTest) that RED under bare `./gradlew test` — they need the H2/DB context the compose stack provides. Extending CI past the YU13–15 engine suites needs a DB service or a Testcontainers/H2-DDL fixture or they red the pipeline.** | high (rest) | 04-CI done |
