...
| [02](02-test-coverage-inventory.md) → [MAP](02-RESULT-coverage-map.md) · **[LIVE COVERAGE](full-test-coverage.md)** | Test-coverage inventory — **✅ DONE 2026-07-24** (`e3225c4f`) | **853 tests, all operator-driven, ZERO in CI** | done |
...
| [05](05-opentelemetry-observability.md) → [RESULT](05-RESULT-opentelemetry-observability.md) | **OpenTelemetry — traces DONE + PROVEN ON KIND 2026-07-28** (`7e50defc`, `8d149930`, `3450532f`, `1792c1f2`; propagated `64dd08d9` YU14, `fb18647a` YU13). **Headline: one order = one trace spanning gateway → consensus commit → apply, with ZERO trace context in the replicated log.** The consensus boundary is crossed by DERIVING trace id + parent span id + sampling verdict on both sides from the client idempotency key the log already carries (`InputEvent.priceTicks`) — no schema change, no member roll, replay still byte-identical. Hot path = 8 longs into a pre-allocated Agrona ring, no lock/alloc/IO and **no backpressure path to the caller**; full ring drops-and-counts, exporter is a separate daemon thread, dead collector costs a counter not a millisecond. Deliberately NOT the OTel SDK (allocates per span on a path under an allocation gate + Epsilon GC) — hand-rolled OTLP/HTTP JSON, zero new deps. **Two of the three "missing" pieces already existed**: the collector, Tempo, Loki, Prometheus, Grafana and promtail have shipped since state 007 — what was missing was any emitter, and that Prometheus **never scraped the cluster tier at all**, so `traderx_cluster_next_order_ref` reached no dashboard. Now scraped per-pod (leader is unanswerable under a round-robin Service scrape) and **directly, not via the collector** — a collector outage must never blank the ground-truth counter. Falsifiable proof `scripts/proofs/yu13-otel-trace-join.sh` predicts the trace id AND parent span id from the ClOrdID alone and demands Tempo return exactly that joined trace (3/3, `4c1a07875de57968` predicted == observed). Suites **328/311/297**, 0 fail, allocation + noGc gates green; apply path byte-identical when tracing is off. **⚠️ Shipped with a real bug, found by checking the kdb lane's disk finding against this design and fixed (`4435d16b`/`c7f85ea6`/`09629695`): the sink bounded memory and bounded the trade path, but NOT the exporter thread — a permanently-full ring made it post back-to-back and burn a core, on the tainted core-pinned member nodes where CPU is the reserved resource. Capped at `OTEL_MIN_INTERVAL_MS` (51.2k spans/s) — **then the cap itself had the same defect one level down** (`0790c66b`/`1a4bcbe2`/`3a209ae0`): it slept AFTER `post()`, so a throwing post skipped the sleep and an unreachable collector tight-spun a pinned core for the whole outage. Pacing is now unconditional + backs off. Suites **330/313/299**. **Generalisation both lanes needed: bounding the queue is not bounding the consumer — and ask it of the failure path, which is the one that persists.** **FOLLOW-UPS DONE + PROVEN ON A 3-MEMBER KIND RIG 2026-07-29 — the doc's open items are now closed.** (a) **Logs joined to traces, both ways.** The open item undersold the gap: the cluster tier had NO per-order log lines at all, and no logging framework either (`System.out`, no logback/log4j config anywhere), so "extend the log pattern" and "use the MDC" were both non-answers. The line worth having is the REJECT, and it carries `trace=<32 hex>` **derived** from the idempotency key exactly as the spans derive theirs — no plumbing, nothing in the log. Trace id goes IN THE LINE, not a Loki label (one stream per order = 190k streams/s at the ceiling) and not structured metadata (deployed Loki 2.9.8 ships `allow_structured_metadata: false`); in-the-line is also the only form that works BOTH ways — `derivedFields` log→trace, `tracesToLogsV2` trace→log. Bounded at `REJECT_LOG_PER_SEC=20` with the suppressed count exported, because **stdout is the one sink here with no overflow valve** (spans drop at a full ring; a log line goes straight to disk) and a 10k/s reject storm is a demonstrated state of this system. **Also found: "Logs for this span" was doubly broken** — nothing to correlate to, AND the Tempo datasource carried the v1 `tracesToLogs` block with no tags and no query, so it had no selector to build. Third piece of provisioned-but-never-exercised wiring on this deliverable. (b) **Error sampling: the answer was NOT a collector tail_sampling processor, and shipping one would have been theatre** — a tail sampler only chooses among spans it RECEIVED, and an unsampled order emits nothing at all, so it could only ever re-filter the 1-in-128 head sampling already kept and would never surface one dropped reject. Instead **rejects are escalated AT THE HEAD**, which works for the same reason the trace joins at all: the committed ack kind is deterministic replicated output, the member produces the byte and the gateway reads the identical byte, so both tiers escalate **together** with nothing carried. Cost: one extra `nanoTime` per order per tier, only while `OTEL_TRACES=1`. **"Slow" is deliberately out of reach** — latency is per-host and non-deterministic, so escalating on it would have one side emit and the other not (a half-trace, worse than none); that is the boundary of the technique: derive-don't-carry can escalate on anything the LOG decides and nothing the WALL CLOCK decides. Falsifiable proof `scripts/proofs/yu13-otel-reject-trace-log-join.sh` runs with head sampling genuinely on and submits two orders that BOTH fail the head verdict — the rejected one must return a whole 5-span trace parented to the PREDICTED consensus span, the accepted one must **404** (the negative case that stops a build quietly tracing everything from passing), Loki must return that order's own line for the predicted id, and the RUNNING Grafana must have the join provisioned both ways. **Cost benchmark ✅ DONE ON GKE 2026-07-28 ([07-RESULT-gke-cost-benchmarks.md](07-RESULT-gke-cost-benchmarks.md)): no measurable cost — throughput −1.5% median (spreads overlap), latency p50/p99 indistinguishable at 1k/5k, spans verified live at the collector.** | high | ✅ done |
| [06](06-kdb-journaling-playback.md) | **KDB-X tick store — DONE 2026-07-27, both halves** ([06-RESULT-kdb-live-capture.md](06-RESULT-kdb-live-capture.md); unpushed, on YU13/YU14/YU15). **Half 1: KDB-X reads the existing ZSTD Parquet NATIVELY — no conversion step, the store IS the corpus in GCS** (16 GiB cap not binding: 768 MiB peak over 47.8M quotes); `tickstore.q` + `selfcheck.q` 17/17, every value cross-computed in DuckDB. **Half 2: kdb now captures the LIVE flow.** `KdbTapWriter` — leader-side, off-consensus, best-effort tap beside `TradeNatsPublisher`/`OrderNatsPublisher`, gated on `KDB_TAP_DIR`, never in the apply path, visible drop signal. New tables are `txOrder`/`txTrade` — NOT `quote`/`trade`, which stay the TAQ tape; both stores load side by side. **Proven on kind (3 members, real REST orders): leader captured 9 order events + 4 executions, both followers header-only, capture count == the cluster's own `trades: 4`, fill VWAP 150.1444 computed in KDB-X off the pulled capture.** Also proven through a real Aeron cluster in the suite (`AeronClusterSpikeTest`, asserted against `engine().tradeCounter()`), which writes the fixture for `txselfcheck.q` (18/18, verified falsifiable). YU14/YU15 service overrides wired and counted in the generated tree (dead-layer trap). **Enabled cost MEASURED ON GKE 2026-07-28 ([07-RESULT-gke-cost-benchmarks.md](07-RESULT-gke-cost-benchmarks.md)): zero at saturation — 131k/126k committed/s with the tap capturing 9.3M rows + 200k trades (~880 MB) per flood; steady-state latency identical to base; post-flood backlog drain bounds the only observable window.** | medium | ✅ done |
