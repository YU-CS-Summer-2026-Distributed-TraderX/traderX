...
|---|---|---|---|---|---|
| 0 | YU01-lmax-sequencer | ✅ merge (**3-conflict subset** — pre-k8s, no state-010) + PR-only gates | ✅ java push-down (1 build.gradle; no npm — reference-data is a later feature) | ⚠️ pre-existing compile break (**baseline-confirmed**) | Added late — the lineage root, missed in the YU02→YU15 pass (yaakov caught it). `ProjectorHandler` ctor mismatch in stale benchmark tests; pre-merge tip fails identically. **Also hit Surprise #6 for real:** the merge broke YU01's *standalone* 009b render — the 009b overlay patch's `RUN_FROM_GENERATED.md` doc hunk had stale context vs upstream's refreshed generated doc; dropped the one cosmetic hunk (57 file sections intact) → renders clean, CVEs land. `lmax-sequencer-no-gc` is the same commit as YU01's old tip (0 divergence) — a superseded alias, not a maintained variant, so **no separate rebase** (fast-forward it to rebased YU01 if the name must stay current — yaakov's call). |
