...
| **Prod database Deployment uses `emptyDir`** — any pod reschedule (node upgrade, eviction, crash) loses ALL read-model data; only the order-matcher journal has a PVC | fixed 2026-07-16 (main session): 10Gi PVC + Recreate in all 5 deployment copies, `900-migrations.sql` idempotent desired-state DDL per configmap layer, `schema-migrate` initContainer for populated volumes (entrypoint skips init-SQL on a populated datadir — a PVC without migrations silently freezes schema) | Fixed + kind-verified (reschedule survival, regressed-schema auto-migration, E2E order). Committed all 8 branches, not pushed; GKE rollout pending the morning deploy |
| **order-matcher startup probe kill-loops any recovery longer than 2 minutes** — diagnosis settled 2026-07-16 with kubelet events: engine init is synchronous (`afterPropertiesSet`), so HTTP serves NOTHING during replay ("connection refused", not an early-UP liveness endpoint) and the 24×5s=120s startup budget was the kill mechanism, not a post-start liveness kill. Fixed 2026-07-16: startupProbe → readiness target, failureThreshold 120 (10-min ceiling), timeoutSeconds 5 on all probes (1s default SIGTERM'd a busy pod 2026-07-02), applied to all FOUR manifest copies (YU02 base, YU02 tilt, YU09, **cluster-addons GKE StatefulSet** — the copy earlier sweeps missed) | fixed 2026-07-16 (main session); kind replay kill-test pending | Fixed pending verification. Still open: a fixed ceiling can't distinguish slow-but-advancing from hung — recovery-progress health is the durable fix. Related: execution-algo-engine ships NO probes at all (dead JVM stays "Ready" — bit us live 2026-07-15) |
| **DB warm-up recovery (`recovery.source=db`) OOMs on a populated database** — root cause of the 2026-07-16 wedged kind rollout (exit 1, `OutOfMemoryError` in `StatefulPersistenceContext.addEntry` at ~53s): the warm-start loads the ENTIRE read model as Hibernate entities in one stateful persistence context. Landmine: code default AND both manifest layers (YU02, YU09 kubernetes-runtime) ship `db`, so any fresh apply against a benchmark-era DB crash-loops; kind only works because `RECOVERY_SOURCE=journal` was set live (GKE prod sets journal in the StatefulSet). Fix (source): stream the warm-up — stateless session / keyset pagination + periodic clear. Design question (user): should the manifest default flip to `journal`? | found 2026-07-16 (main session), evidence in kubelet events + crash logs; no doc yet | Open — assigned to codeX overnight lane (order-matcher source) |
| **Generated YU02's Spring tests fail — `Table "ORDERBOOK" not found`** (13 of 38, one root cause: `LmaxEngine` warm-reads `FROM OrderBook` under `recovery.source=db` in the `ddl-auto=create-drop` H2 test context). NOT a regression and NOT a production defect — YU02 hadn't compiled since 2026-07-14, so the suite simply hadn't run; production gets its schema from the `database-init` ConfigMap. Same warm-up path as the row above. **Unexplained:** generated YU15 passes 340/0 with identical `recovery.source`, identical `LmaxEngine` recovery guard, identical `DbWarmupReader` call site and near-identical test properties — no theory should be believed until it accounts for that. Two fixes attempted and both wrong (backport a `journal` default — no layer has one; `@DependsOn("entityManagerFactory")` — composed correctly, changed nothing), so it is not DDL running *late*. Next step is a diagnostic, not a theory: dump `INFORMATION_SCHEMA.TABLES` / the emitted `create table`. Leading hypothesis is H2 `MODE=MySQL` identifier casing (quoted `"OrderBook"` vs unquoted `ORDERBOOK`). Do **not** make the warm-up tolerate a missing table — that trades a loud recovery failure for silent empty start | `HANDOFF-issue-yu02-spring-tests-orderbook-table.md` | ROOT CAUSE FOUND 2026-08-03 (via the YU01 fix): Spring Boot camel-hump naming creates `order_book` while the projector writes `orderbook`, the name mariadb-init makes. H2-casing and SQL-dialect hypotheses both DISPROVEN. Fix = give the YU02 layer a test schema matching production, as YU01 now has (YU01 ships 1 .sql, YU02 ships 0). Unverified for YU02 — re-run the suite before closing |
| **AllocationGateTest: intermittent exactly-72-byte "allocation" (NGC-01)** — ROOT-CAUSED 2026-07-16: C2 escape-analysis **rematerialization at a mid-window deoptimization**, not a hot-path allocation. Evidence: value always exactly 72 = sizeof(InputEvent); with `-XX:-UseTLAB` + JFR ObjectAllocationOutsideTLAB (exact recording), a captured flake shows ZERO Java allocations JVM-wide during the window while the counter moved 72B — deopt reifies a scalar-replaced InputEvent, bumping allocated-bytes with no JFR-visible event. Fixed: `-Xbatch` on the normal-GC gate JVMs (compilation completes pre-window; 10/10 + 5/5 deterministic vs ~30–80% flake). Assertions unchanged — **NGC-01 is genuinely intact**; the gate was measuring a compiler artifact. **2026-08-24: the "deterministic" claim has an unstated LOAD precondition.** `riskAllocationGateTest` flipped 72 → 0 across two back-to-back runs of one commit (`82292970`) on a machine another lane was deliberately loading — with `-Xbatch -XX:-TieredCompilation -XX:CompileThreshold=10000` all present. Contention delays compilation past the window the mitigation exists to clear, so the mitigation degrades exactly when the machine is busy. Not a new defect and not worth its own file (CI's `retry_isolated` already covers it); the new fact is that a LOCAL red here is uninformative unless the box was quiet — check load before believing it | root-caused + fixed 2026-07-16 (main session); committed all 8 branches | Closed — deterministic on a QUIET box; load-sensitive, see the 2026-08-24 note |
...
| **Journal archival: upload leg dead in prod, disk unbounded** — YU09 rotation is active on GKE but the `order-matcher-journal-gcs-hmac` Secret was never provisioned, so the fail-safe (never delete an unconfirmed segment) accumulates immutable segments on the PVC forever. Mitigation landed 2026-07-16 (`78c5d27`): rotation-cadence disk-watermark alerting in JournalArchiver (WARN ≥80%, ERROR ≥90%, Loki-alertable). Remaining: provision the HMAC secret (user; gcloud→kubectl pipeline that never prints key material was handed over) | found via critique review 2026-07-15; watermark guard + startup resubmit sweep (`02c97d3`) committed and secret provisioned 2026-07-16 | **Closed** — verified live: 208 stranded segments drained to GCS on the first post-deploy startup (local count 0, bucket at 228 objects); rotation-cadence watermark alerting guards regressions |
