...
## Resolved 2026-08-24 — the adapter is deployed, fixed, and sequencing

Commits `a7ec2c8b` (parse + keepalive + seeder) and the `:yu17-tick2` wiring commit. What closed it:

- **Deployed**: `feed-adapter` Deployment at `replicas: 1` on `traderx/cluster-node:yu17-tick2`,
  one tag across members/gateway/risk-extract/adapter, manifests declaring it.
- **Three lifelong defects fixed in `FeedAdapterMain`**, each found only by running it: the envelope
  parse (`payload.price`), the missing session keepalive (idle session expired, offer() spun on
  CLOSED), and the exit that wasn't (a throw out of main leaves the JVM alive on non-daemon NATS/
  Aeron threads — 1/1 Running, sequencing nothing; now `System.exit(1)`). Self-heal verified in
  anger: **7 kubelet restarts** across one full suite run's rolling proofs, resuming within a flush
  each time.
- **The discriminating pair**, same rig, same 90s, algo engine at 0, no client traffic:
  adapter scaled to 0 → `traderx_cluster_applied` 1026 → 1026 (**delta 0**);
  adapter at 1 → 1026 → 1368 (**delta 342**), all three members agreeing.
- **End to end with no manual seed**: `BUY NVDA @ 917.44` (its published price; fixtures used to
  pin it at 200) → accepted, on an epoch whose only price inputs were the seeder's one-time
  live-price pass and the adapter's ticks.
- **The full suite is stable with ticks flowing**: 26/29 green across runs 2–3
  (`yu13-stp-and-replace` green after its boundary-pair rebuild). The two residuals are not
  price-shaped: the yu04 pair is NATS crash damage
  (`issues/open/the-control-stream-lost-its-universe-in-the-disk-crash.md`) and
  `yu16-treasury-pricing` asserts the ADR-068 FRED curve (commit `6f781558`, another lane, landed
  mid-run) against a deployed `price-publisher:yu16` image that predates it — that lane's rebuild.

The band now follows the market, and the market now reaches the log.
