...
## Resolved 2026-08-24 (commit `a7ec2c8b`, proven in the full suite the same day)

The fix went one step wider than filed, because the equity `/seed` block at a flat 200 was the same
defect one layer up — every equity's sequenced reference, not just the three crossings:

- the whole quoted universe (equities included) now gets the live-price re-seed the other classes
  already had; the `t in seeded` exclusion is gone and the census gained an `equity` class;
- `hold()` crosses at the live price via `live_px()` over the same feed read, 200 kept only as the
  fallback when the feed is unreadable;
- a refused `hold()` leg is now **fatal** — it used to `-o /dev/null` both legs and print "holds"
  over a 422, which is how this class stayed invisible;
- `yu10-fix-session` reads IBM's live price the way `yu13-cancel-ingress` does (the latent half).

Proven on `kind-traderx-yu12-cluster` with the feed adapter live and sequencing: the seeder logged
`10031 holds NVDA x25 @ 945.252` on a fresh epoch (the same crossing that was refused PRICE_COLLAR
at 200 with a tick in play), and the proofs that need the holdings — `yu06-consumer-halt`,
`yu05-recon`, `yu05-settlement`, `yu15-risk-extract` — all passed in the full-suite run
(25 passed / 2 skipped / 2 failed, no failure price-shaped).
