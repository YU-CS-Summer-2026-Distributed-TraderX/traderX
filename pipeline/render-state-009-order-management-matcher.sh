...
# Preserve our benchmark-dashboard requirement that upstream's normalize script does not set:
# sub-5s dashboard auto-refresh (Grafana's default min_refresh_interval is 5s) so the trades/sec
# dashboard updates at the 1s Prometheus scrape cadence. Idempotent; anchors on the sub-path line
# that normalize-observability-runtime.sh guarantees is present.
