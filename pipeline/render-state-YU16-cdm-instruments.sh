...
STATE_SPEC_DIR="${ROOT}/specs/YU16-cdm-instruments"
GENERATED_ROOT="${TRADERX_GENERATED_ROOT:-${ROOT}/generated}"
TARGET_ROOT="${GENERATED_ROOT}/code/target-generated"
PARENT_STATE_DIR="${TARGET_ROOT}/YU15-eod-risk-extract"
STATE_DIR="${TARGET_ROOT}/YU16-cdm-instruments"
SPEC_SOURCE_DIR="${STATE_DIR}/spec-source"
RUNTIME_OVERRIDES_DIR="${STATE_SPEC_DIR}/generation/runtime-overrides"

[[ -d "${PARENT_STATE_DIR}" ]] || {
  echo "[fail] required parent YU15-eod-risk-extract artifact missing for YU16 render: ${PARENT_STATE_DIR}"
  exit 1
}
...
overlay_dir() {
  ...
  if [[ -d "${src}" ]] && find "${src}" -type f -print -quit | grep -q .; then
    ...
    echo "[info] no ${label} overrides present (${src}); keeping YU15-eod-risk-extract parity"
  fi
}
...
# YU16-cdm-instruments Generated Artifacts
...
- parent state: `YU15-eod-risk-extract` (which renders onto `YU14-listed-equity-options` ->
